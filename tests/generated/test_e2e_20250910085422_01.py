...
def test_app_shell_context_and_sqlalchemy_extension(tmp_path):
    # Imports inside test as required
    from conduit.app import create_app
    from conduit.settings import TestConfig
    import flask

    # Create app using TestConfig
    app = create_app(TestConfig)

    # Basic sanity checks
    assert isinstance(app, flask.Flask)

    # Shell context should be a dict (may be empty or populated)
    ctx = app.make_shell_context()
    assert isinstance(ctx, dict)

    # The application should have registered the SQLAlchemy extension
    # Flask-SQLAlchemy typically registers under the 'sqlalchemy' key
    assert isinstance(app.extensions, dict)
    assert 'sqlalchemy' in app.extensions

def test_404_routes_return_json_error():
    from conduit.app import create_app
    from conduit.settings import TestConfig
    ...
    # Request an unknown route and ensure the app's error handlers produce JSON
    rv = client.get('/this-route-should-not-exist-xyz')
    assert rv.status_code == 404
    # Ensure content type indicates JSON
    ct = rv.content_type or ''
    assert 'application/json' in ct or 'json' in ct
    # Body should parse as JSON to a dict
    body = rv.get_json(silent=True)
    assert isinstance(body, dict)

def test_app_has_api_blueprint_prefix_rules():
    from conduit.app import create_app
    from conduit.settings import TestConfig

    app = create_app(TestConfig)

    # The project registers its API blueprints under an '/api' prefix in typical setups.
    # Verify that at least one rule uses the '/api' prefix to ensure blueprints were registered.
    rules = list(app.url_map.iter_rules())
    assert any(rule.rule.startswith('/api') for rule in rules)
