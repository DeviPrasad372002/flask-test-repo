...
def _call_attr(obj, *names):
    for n in names:
        ...
        if attr is not None:
            return attr
    raise AttributeError(f"None of attributes {names} found on {obj!r}")

def _call_maybe_callable(val, *args, **kwargs):
    if callable(val):
        return val(*args, **kwargs)
    return val

def test_favorite_and_unfavorite_workflow():
    from conduit.app import create_app
    from conduit.settings import TestConfig
    from conduit.extensions import db
    # model imports
    from conduit.user.models import User
    from conduit.articles.serializers import make_article
    ...
    with app.app_context():
        ...
        # create two users: author and other_user
        author = User(username="author", email="author@example.test")
        # try common password setter if exists
        if hasattr(author, "set_password"):
        ...
        # create an article by author using serializer helper
        art_data = {"title": "Hello", "description": "desc", "body": "body"}
        article = make_article(art_data, author)
        db.session.add(article)
        db.session.commit()

        # get favoritesCount accessor (call if callable)
        fav_cnt_attr = getattr(article, "favoritesCount", None)
        fav_count = _call_maybe_callable(fav_cnt_attr)
        assert fav_count == 0

        # favourite (try british/us spellings)
        fav_method = _call_attr(article, "favourite", "favorite")
        fav_method(other)
        db.session.commit()

        # is_favourite should reflect change
        is_fav = _call_attr(article, "is_favourite", "is_favorite")(other)
        assert bool(is_fav) is True

        fav_count_after = _call_maybe_callable(getattr(article, "favoritesCount"))
        assert fav_count_after == 1

        # unfavourite
        unfav_method = _call_attr(article, "unfavourite", "unfavorite")
        unfav_method(other)
        db.session.commit()

        is_fav_after = _call_attr(article, "is_favourite", "is_favorite")(other)
        assert bool(is_fav_after) is False

        fav_count_final = _call_maybe_callable(getattr(article, "favoritesCount"))
        assert fav_count_final == 0

def test_comment_create_and_delete_workflow():
    from conduit.app import create_app
    from conduit.settings import TestConfig
    from conduit.extensions import db
    from conduit.user.models import User
    from conduit.articles.serializers import make_article, make_comment
    ...
    with app.app_context():
        ...
        # create author and commenter
        author = User(username="article_author", email="aa@example.test")
        commenter = User(username="commenter", email="comm@example.test")
        if hasattr(author, "set_password"):
            ...
            try:
                ...
                pass
        if hasattr(commenter, "set_password"):
            commenter.set_password("x")
        else:
            try:
                commenter.password = "x"
            except Exception:
                pass
        ...
        # create article
        art = make_article({"title": "C Test", "description": "d", "body": "b"}, author)
        db.session.add(art)
        db.session.commit()

        # create a comment via serializer helper
        comment_payload = {"body": "Nice article!"}
        comment = make_comment(comment_payload, commenter, art)
        db.session.add(comment)
        db.session.commit()

        # Verify comment appears in DB and is linked to article
        all_comments = Comment.query.all()
        # filter comments for this article by comparing article id or relationship
        def belongs_to_article(c):
            # try relationship attribute first
            try:
                if getattr(c, "article", None) is art:
                    return True
            except Exception:
                pass
            # fallback to id comparison
            a_id = getattr(c, "article_id", None)
            if a_id is None:
                # try alternative naming
                a_id = getattr(c, "articleId", None)
            return a_id == getattr(art, "id", None)

        linked = [c for c in all_comments if belongs_to_article(c)]
        assert len(linked) == 1
        stored = linked[0]
        assert getattr(stored, "body", None) == "Nice article!"

        # delete the comment (use model delete if present, else session delete)
        delete_method = getattr(stored, "delete", None)
        ...
        # ensure deleted
        remaining = [c for c in Comment.query.all() if belongs_to_article(c)]
