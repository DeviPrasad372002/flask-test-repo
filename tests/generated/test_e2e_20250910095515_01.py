...
def _call_maybe(obj, name, *args, **kwargs):
    attr = getattr(obj, name)
    return attr(*args, **kwargs) if callable(attr) else attr

def _names_from_tags(tags):
    names = []
    for t in tags or []:
        if hasattr(t, "name"):
            names.append(t.name)
...
def test_article_favourite_unfavourite_e2e(tmp_path):
    # Import targets inside the test to satisfy dev instructions
    ...
    # Ensure a clean sqlite in-memory DB for determinism if TestConfig doesn't do so
    ...
    with app.app_context():
        ...
        # Create a user
        u = User(username="tester", email="tester@example.org", password="secret")
        db.session.add(u)
        db.session.commit()

        # Create an article authored by the user
        # Try typical constructor; if it requires different args this will raise and fail clearly
        a = Article(title="E2E Title", description="desc", body="body", author=u)
        db.session.add(a)
        db.session.commit()

        # Helper to obtain favorites count (support attribute or callable)
        fav_count_before = _call_maybe(a, "favoritesCount")
        assert fav_count_before == 0

        # Favourite the article
        # Some implementations return the article or None; we commit afterwards to persist
        res = _call_maybe(a, "favourite", u)
        db.session.add(a)
        db.session.commit()

        # Reload from DB to avoid stale relationships
        fresh = Article.query.get(a.id)
        assert _call_maybe(fresh, "is_favourite", u) or getattr(fresh, "favorited", None) is not None
        assert _call_maybe(fresh, "favoritesCount") == 1

        # Unfavourite and verify counts go back to zero
        _call_maybe(fresh, "unfavourite", u)
        db.session.add(fresh)
        db.session.commit()

        fresh2 = Article.query.get(a.id)
        assert not _call_maybe(fresh2, "is_favourite", u)
        assert _call_maybe(fresh2, "favoritesCount") == 0

def test_article_tag_add_remove_e2e(tmp_path):
    from conduit.app import create_app
    from conduit.settings import TestConfig
    from conduit.extensions import db
    from conduit.user.models import User
    ...
    with app.app_context():
        ...
        user = User(username="tagger", email="tagger@example.org", password="pwd")
        db.session.add(user)
        db.session.commit()

        article = Article(title="Tag Title", description="d", body="b", author=user)
        db.session.add(article)
        db.session.commit()

        # Add tags
        _call_maybe(article, "add_tag", "python")
        _call_maybe(article, "add_tag", "flask")
        # Adding same tag again should be idempotent in many implementations
        _call_maybe(article, "add_tag", "python")
        db.session.add(article)
        db.session.commit()

        fresh = Article.query.get(article.id)
        tag_names = _names_from_tags(getattr(fresh, "tags", []))
        ...
        # Remove a tag
        _call_maybe(fresh, "remove_tag", "python")
        db.session.add(fresh)
        db.session.commit()

        fresh2 = Article.query.get(article.id)
        tag_names_after = _names_from_tags(getattr(fresh2, "tags", []))
        assert "python" not in tag_names_after
        assert "flask" in tag_names_after
