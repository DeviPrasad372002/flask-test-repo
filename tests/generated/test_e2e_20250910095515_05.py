...
def _exc_lookup(name, fallback):
    # Try to find an exception class in conduit.exceptions; fall back to provided fallback
    try:
...
def test_create_calls_db_add_and_commit(monkeypatch):
    # Import target module inside the test
    import conduit.extensions as ext

    # Replace ext.db with a fake db that has a session
    fake_session = make_fake_session()
    fake_db = types.SimpleNamespace(session=fake_session)
    monkeypatch.setattr(ext, "db", fake_db, raising=True)

    # Define a simple fake model class
    class FakeModel:
    ...
    # Attempt to call the create helper. Accept either signature ext.create(Model, **kwargs)
    # or ext.create(instance) — try both patterns deterministically.
    created = None
    try:
        created = ext.create(FakeModel, name="alice")
    except TypeError:
        # fallback: ext.create expects an instance
        inst = FakeModel(name="alice")
        created = ext.create(inst)

    # Verify that an instance was returned and session saw the object added and committed
    assert isinstance(created, _exc_lookup("Exception", Exception))
    assert getattr(created, "name", None) == "alice"
    assert fake_session.added, "expected db.session.add to be called"
    # ensure the exact object was added or at least an object exists
    assert any(isinstance(o, _exc_lookup("Exception", Exception)) for o in fake_session.added)
    assert fake_session.committed >= 1

def test_update_sets_attributes_and_commits(monkeypatch):
    import conduit.extensions as ext
    ...
    inst = FakeModel(title="old", count=1)

    # Try update in both common signatures: ext.update(instance, **kwargs) or ext.update(instance, mapping)
    updated = None
    try:
        updated = ext.update(inst, title="new", count=2)
    except TypeError:
        # fallback: maybe requires a dict
        updated = ext.update(inst, {"title": "new", "count": 2})

    # If update returns None, we still check the instance was mutated and commit called
    if updated is None:
        updated = inst

    assert getattr(updated, "title", None) == "new"
    assert getattr(updated, "count", None) == 2
    assert fake_session.committed >= 1

def test_save_adds_and_commits_and_handles_exceptions(monkeypatch):
    import conduit.extensions as ext
    ...
    inst = FakeModel(foo="bar")

    # Call save; many implementations return the instance
    result = None
    try:
        result = ext.save(inst)
    except Exception as e:
        # If an error occurs, ensure it's a known/expected exception type from conduit.exceptions if defined.
        exccls = _exc_lookup("Exception", Exception)
    ...
    # Ensure save attempted to add and commit
    assert any(o is result for o in fake_session.added)
    assert fake_session.committed >= 1
