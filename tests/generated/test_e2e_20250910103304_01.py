...
def test_create_app_registers_extensions():
    # Import inside test to follow constraints
    from conduit.app import create_app

    app = None
    # try several common ways the factory might accept configuration
    for arg in (None, 'testing', 'TestConfig', 'Test'):
        try:
            app = create_app() if arg is None else create_app(arg)
            break
        except Exception:
            app = None
    assert app is not None, "create_app failed to construct an app with common config names"

    with app.app_context():
        exts = getattr(app, "extensions", {})
        assert isinstance(exts, _exc_lookup("Exception", Exception))
        # ensure at least one expected extension key is present
        assert any(k in exts for k in ("sqlalchemy", "migrate", "bcrypt", "jwt", "cache", "cors"))

def test_shell_context_provides_db_and_models():
    # Import the shell_context provider directly and inspect its mapping
    from conduit.app import shell_context

    ctx = shell_context()
    assert isinstance(ctx, _exc_lookup("Exception", Exception))

    # There should be at least one database-like object and one model in the context
    has_db = "db" in ctx and hasattr(ctx["db"], "session") or any(hasattr(v, "session") for v in ctx.values())
    has_model = any(name in ctx for name in ("User", "Article", "Comment", "Tags"))
    assert has_db, "shell context did not expose a DB-like object with .session"
    assert has_model, "shell context did not expose at least one expected model (User, Article, Comment, Tags)"

def test_errorhandler_decorator_returns_callable_and_app_stays_healthy():
    # Import inside test
    from conduit.app import errorhandler, create_app

    # errorhandler should return a decorator when given an Exception subclass
    decorator = None
    try:
        decorator = errorhandler(Exception)
    except Exception:
        # If it refuses Exception, try a more specific built-in exception
        decorator = errorhandler(ValueError)

    assert callable(decorator), "errorhandler(...) did not return a callable decorator"

    # Use the decorator on a simple function; result should be callable (registration or wrapper)
    def dummy(exc):
        return ("dummy", 500)
    wrapped = decorator(dummy)
    assert callable(wrapped)

    # Ensure creating app still works after interacting with decorator
    app = None
    for arg in (None, 'testing', 'TestConfig'):
        try:
            app = create_app() if arg is None else create_app(arg)
            break
        except Exception:
    ...
    # Basic smoke test: ensure test client can be created
