...
def _find_in_mapping(obj, key):
    if isinstance(obj, _exc_lookup("Exception", Exception)):
        if key in obj:
            return obj[key]
        for v in obj.values():
            res = _find_in_mapping(v, key)
            if res is not None:
                return res
    elif isinstance(obj, _exc_lookup("Exception", Exception)):
        for item in obj:
            res = _find_in_mapping(item, key)
            if res is not None:
                return res
    return None

def _call_if_callable(maybe, *args, **kwargs):
    return maybe(*args, **kwargs) if callable(maybe) else maybe

def test_article_tags_and_favorites_lifecycle(tmp_path):
    # Import framework and models inside test for isolation
    ...
    # Create app and initialize extensions (deterministic TestConfig)
    app = Flask(__name__)
    ...
    with app.app_context():
        ...
        # create an author and a reader
        author = User(username='author', email='author@example.com', password='pass')
        reader = User(username='reader', email='reader@example.com', password='pass')
        db.session.add_all([author, reader])
        db.session.commit()

        # create an article by author
        article = Article(title='E2E tags & favs', description='desc', body='body', author=author)
        db.session.add(article)
        db.session.commit()

        # add tags
        # use add_tag method; tolerate either method or attribute
        add_tag = getattr(article, 'add_tag', None)
        assert add_tag is not None, "Article model missing add_tag"
        _call_if_callable(add_tag, 'python')
        _call_if_callable(add_tag, 'testing')
        db.session.commit()

        # ensure tags present
        tags = getattr(article, 'tags', None)
        assert tags is not None
        tag_names = [getattr(t, 'name', None) for t in tags]
        assert 'python' in tag_names and 'testing' in tag_names

        # favourite by reader
        fav_method = getattr(article, 'favourite', None) or getattr(article, 'favorite', None)
        assert fav_method is not None
        _call_if_callable(fav_method, reader)
        db.session.commit()

        # check favoritesCount and is_favourite
        fav_count_accessor = getattr(article, 'favoritesCount', None) or getattr(article, 'favorites_count', None)
        assert fav_count_accessor is not None
        fav_count = _call_if_callable(fav_count_accessor)
        assert int(fav_count) == 1

        is_fav = getattr(article, 'is_favourite', None)
        assert is_fav is not None
        assert _call_if_callable(is_fav, reader) is True

        # unfavourite and confirm
        unfav = getattr(article, 'unfavourite', None) or getattr(article, 'unfavorite', None)
        assert unfav is not None
        _call_if_callable(unfav, reader)
        db.session.commit()
        fav_count_after = int(_call_if_callable(fav_count_accessor))
        assert fav_count_after == 0

        # remove a tag and confirm
        remove_tag = getattr(article, 'remove_tag', None)
        assert remove_tag is not None
        _call_if_callable(remove_tag, 'testing')
        db.session.commit()
        tag_names_after = [getattr(t, 'name', None) for t in article.tags]
        assert 'testing' not in tag_names_after
        assert 'python' in tag_names_after

def test_serialization_roundtrip_article_and_comment(tmp_path):
    from flask import Flask
    from conduit.settings import TestConfig
    from conduit.app import register_extensions
    from conduit.database import db
    ...
    from conduit.articles import serializers
    ...
    with app.app_context():
        ...
        author = User(username='ser', email='ser@example.com', password='pw')
        commenter = User(username='comm', email='comm@example.com', password='pw')
        db.session.add_all([author, commenter])
        db.session.commit()

        art = Article(title='Serialize Me', description='d', body='b', author=author)
        db.session.add(art)
        db.session.commit()

        # add a tag and a comment
        _call_if_callable(getattr(art, 'add_tag'), 'api')
        db.session.commit()

        comment = Comment(body='Nice article', author=commenter, article=art)
        db.session.add(comment)
        db.session.commit()

        # use dump_article and dump_comment; these should return mappings that include the title/body
        dump_article = getattr(serializers, 'dump_article', None)
        dump_comment = getattr(serializers, 'dump_comment', None)
        assert dump_article is not None and dump_comment is not None

        dumped_article = dump_article(art)
        dumped_comment = dump_comment(comment)

        # defensively search for known values
        found_title = _find_in_mapping(dumped_article, 'title')
        found_body = _find_in_mapping(dumped_article, 'body')
        assert found_title == art.title
        assert found_body == art.body

        found_comment_body = _find_in_mapping(dumped_comment, 'body')
        assert found_comment_body == comment.body

        # dump_articles should accept list-like and include our article title somewhere
        dump_articles = getattr(serializers, 'dump_articles', None)
        if dump_articles is not None:
            da = dump_articles([art])
            # find any occurrence of the title
            assert _find_in_mapping(da, 'title') == art.title
        else:
            # If missing, at least ensure the single-article serializer worked above
            assert found_title == 'Serialize Me'
