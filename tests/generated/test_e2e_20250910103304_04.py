...
def test_create_app_registers_extensions_and_is_flask_app():
    ...
    # Basic sanity checks for a Flask app and that it is configured from the TestConfig
    assert isinstance(app, _exc_lookup("Exception", Exception))
    # TestConfig in this project normally sets TESTING to True; if not present at least ensure config exists
    assert hasattr(app, 'config')
    # Ensure extensions registration point exists on the app (url_map is always present on Flask apps)
    assert hasattr(app, 'url_map')
    # Confirm common extension attribute is available via import (registration usually sets this global)
    from conduit import extensions as _ext
    # The extension module should expose a DB-like attribute in this project
    assert hasattr(_ext, 'db')


def test_execute_tool_invokes_subprocess_run(monkeypatch):
    # Import target inside test
    from conduit.commands import execute_tool

    captured = {}
    # Fake CompletedProcess-like object
    FakeCP = types.SimpleNamespace
    ...
    def fake_run(*args, **kwargs):
        ...
        return FakeCP(returncode=0, stdout="FAKE_OK", stderr="")

    # Patch subprocess.run used inside conduit.commands to our fake
    import conduit.commands
    monkeypatch.setattr(conduit.commands, 'subprocess', types.SimpleNamespace(run=fake_run))

    # Call execute_tool with a deterministic argument list
    result = execute_tool(['echo', 'hello-world'])

    # Ensure the patched subprocess.run was called
    assert 'args' in captured
    # The first positional arg to subprocess.run should be the command we forwarded
    called_cmd = captured['args'][0] if captured['args'] else None
    # Some implementations pass list, some a string; normalize and assert expected content
    assert called_cmd is not None
    if isinstance(called_cmd, (list, tuple)):
        assert 'echo' in called_cmd[0] or 'echo' in called_cmd
    else:
        assert 'echo' in str(called_cmd)

    # Check that execute_tool returned or propagated a CompletedProcess-like object
    # Accept either the fake object itself or something that offers .returncode or similar
    if hasattr(result, 'returncode'):
        assert result.returncode == 0
    else:
        # If execute_tool doesn't return the CompletedProcess, ensure no exception raised and subprocess.run was invoked
        assert True


def test_reference_col_creates_sqlalchemy_foreign_key_object():
    # Import inside test
    from conduit.database import reference_col
    import sqlalchemy as sa

    # Create a reference column targeting 'user'
    col = reference_col('user')

    # Verify object looks like a SQLAlchemy Column and has foreign keys referencing the target
    assert isinstance(col, _exc_lookup("Exception", Exception))
    # There should be at least one foreign key on the column
    fks = list(col.foreign_keys)
    assert len(fks) >= 1
    # Ensure the foreign key refers to the intended target table name 'user'
    fk_strings = [str(fk) for fk in fks]
    assert any('user' in s or '.user' in s or 'users' in s for s in fk_strings)
