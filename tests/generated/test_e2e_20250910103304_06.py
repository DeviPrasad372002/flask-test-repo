...
def _find_in_mapping(mapping, key):
    if isinstance(mapping, _exc_lookup("Exception", Exception)):
        if key in mapping:
            return mapping[key]
        for v in mapping.values():
            found = _find_in_mapping(v, key)
            if found is not None:
                return found
    elif isinstance(mapping, (list, tuple)):
        for item in mapping:
            found = _find_in_mapping(item, key)
            if found is not None:
                return found
    return None
...
def _exc_lookup(name, default=Exception):
    try:
        mod = __import__('conduit.exceptions', fromlist=[''])
        return getattr(mod, name, default)
    except Exception:
        return default

def test_invalidusage_to_json_structure():
    # Import inside test
    from conduit.exceptions import InvalidUsage

    # Create an InvalidUsage instance with explicit message and status_code
    exc = InvalidUsage("Something went wrong", status_code=422, payload={"extra": "info"})

    # Many implementations provide a to_json method / property that returns serializable data
    # Be permissive: accept dict-like return value
    to_json = getattr(exc, "to_json", None)
    assert to_json is not None, "InvalidUsage missing to_json method"

    data = to_json()
    assert isinstance(data, _exc_lookup("Exception", Exception)), "to_json should return a dict-like object"

    # Expect at least the message to be present somewhere and the status code as well
    msg = _find_in_mapping(data, "message")
    code = _find_in_mapping(data, "status_code")
    # Some implementations may use 'error' or 'description' instead of 'message'
    if msg is None:
        msg = _find_in_mapping(data, "error") or _find_in_mapping(data, "description")

    assert msg is not None and "Something went wrong" in str(msg)
    assert code is not None and int(code) == 422

def test_profileschema_serializes_userprofile_fields():
    # Import inside test
    from conduit.profile.models import UserProfile
    from conduit.profile.serializers import ProfileSchema

    # Construct a UserProfile with predictable attributes
    profile = UserProfile(username="alice", bio="I am Alice", image="http://img", following=False)

    schema = ProfileSchema()
    # Marshmallow schemas may accept objects directly for dump
    dumped = schema.dump(profile)

    # Ensure we got a mapping back
    assert isinstance(dumped, (dict, list)), "ProfileSchema.dump returned unexpected type"

    # Find username and bio somewhere in the dumped structure
    username = _find_in_mapping(dumped, "username")
    bio = _find_in_mapping(dumped, "bio")
    image = _find_in_mapping(dumped, "image")
    following = _find_in_mapping(dumped, "following")

    assert username == "alice"
    assert bio == "I am Alice"
    assert image == "http://img"
    # following may be present or omitted; if present ensure type is boolean
    if following is not None:
        assert isinstance(following, _exc_lookup("Exception", Exception))

def test_crudmixin_create_triggers_save(monkeypatch):
    # Import inside test
    ...
    # fake save function that records calls and returns the object
    def fake_save(obj):
        called.append(("func_save", obj))
        return obj

    # Monkeypatch the module-level save function
    monkeypatch.setattr(ext_mod, "save", fake_save, raising=True)

    # Define a local model that uses CRUDMixin; implement instance save to call fake_save as well
    class LocalModel(CRUDMixin):
        def __init__(self, **kwargs):
            self.__dict__.update(kwargs)

        def save(self):
            # call the same fake_save so either instance.save() or module.save(obj) will record
            return fake_save(self)

    # Use the mixin to create an instance
    instance = LocalModel.create(foo="bar", number=42)

    # Returned instance should have the attributes set
    assert hasattr(instance, "foo") and instance.foo == "bar"
    assert hasattr(instance, "number") and instance.number == 42

    # Either instance.save() or ext_mod.save(instance) should have been invoked exactly once
    assert len(called) == 1
    marker, obj = called[0]
    assert marker == "func_save"
