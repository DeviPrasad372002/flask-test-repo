...
def test_create_app_uses_test_config_and_shell_context():
    from conduit.app import create_app, shell_context
    from conduit.settings import TestConfig

    app = create_app(TestConfig)

    # Basic expectations about the app configuration for tests
    assert app is not None
    assert app.config.get("TESTING", False) is True

    # The shell_context helper should return a mapping of useful objects
    ctx = shell_context()
    assert isinstance(ctx, _exc_lookup("Exception", Exception))
    # Expect at least db or User to be present in the shell context mapping
    assert any(name in ctx for name in ("db", "User", "Article", "Tag", "Profile")), "shell_context should expose common objects"


def test_article_favourite_unfavourite_lifecycle(tmp_path):
    from conduit.app import create_app
    from conduit.settings import TestConfig
    from conduit.extensions import db as _db_module
    from conduit.user.models import User
    from conduit.articles.models import Article

    app = create_app(TestConfig)
    # Use application context to work with the ORM
    with app.app_context():
        db = getattr(_db_module, "db", None) or _db_module  # support different export styles

        # create all tables
        if hasattr(db, "create_all"):
            db.create_all()

        # helper to persist objects robustly
        def save(obj):
            if hasattr(obj, "save") and callable(getattr(obj, "save")):
                obj.save()
            else:
                # fallback to SQLAlchemy session usage
                db.session.add(obj)
                db.session.commit()

        # Create a minimal user
        u = User()
        # Set commonly expected attributes; tolerate missing ones if model differs
        for attr, val in (("username", "alice"), ("email", "alice@example.com"), ("password", "secret")):
            try:
                setattr(u, attr, val)
            except Exception:
                # ignore attributes that do not exist on the model
                pass
        save(u)
        ...
        for attr, val in (("title", "Test Article"), ("body", "Body"), ("description", "Desc")):
            try:
                ...
                pass
        # Try to attach author relationship if available
        try:
            if hasattr(a, "author") or "author" in getattr(a, "__dict__", {}):
                a.author = u
            else:
                # some models expect author_id
                if hasattr(u, "id") and hasattr(a, "author_id"):
                    a.author_id = getattr(u, "id")
        except Exception:
            pass
        save(a)

        # Utility accessors with fallbacks for different naming conventions
        def call_maybe(obj, *names, default=None, args=(), kwargs=None):
            kwargs = kwargs or {}
            for name in names:
                if hasattr(obj, name):
        ...
        # initial favorites count (0 expected)
        initial = call_maybe(a, "favoritesCount", "favorites_count", "favorited", default=0)
        # If it's a callable (favorited may return list), normalize to int
        if callable(initial):
            initial = initial()
        try:
            initial_val = int(initial)
        except Exception:
            # best-effort: if it's a collection, take its length
            try:
                initial_val = len(initial)
            except Exception:
                initial_val = 0
        ...
        for name in ("favourite", "favorite"):
            if hasattr(a, name):
                fav_fn = getattr(a, name)
                break
        assert fav_fn is not None, "Article model must provide favourite()/favorite()"
        fav_fn(u)

        # ensure saved / committed
        if hasattr(db.session, "commit"):
            db.session.commit()

        # check is_favourite
        is_fav = call_maybe(a, "is_favourite", "is_favorite", default=False, args=(u,))
        if callable(is_fav):
            is_fav = is_fav
        assert (is_fav is True) or (is_fav == True)

        # favorites count increased
        after = call_maybe(a, "favoritesCount", "favorites_count", "favorited", default=1)
        if callable(after):
            after = after()
        try:
            after_val = int(after)
        ...
        for name in ("unfavourite", "unfavorite"):
            if hasattr(a, name):
                unfav_fn = getattr(a, name)
                break
        assert unfav_fn is not None, "Article model must provide unfavourite()/unfavorite()"
        unfav_fn(u)
        if hasattr(db.session, "commit"):
            db.session.commit()

        # ensure is no longer favourited
        is_fav_after = call_maybe(a, "is_favourite", "is_favorite", default=False, args=(u,))
        if callable(is_fav_after):
            is_fav_after = is_fav_after
        assert (is_fav_after is False) or (is_fav_after == False)

        # favorites count back to initial
        final = call_maybe(a, "favoritesCount", "favorites_count", "favorited", default=0)
        if callable(final):
            final = final()
        try:
            final_val = int(final)
