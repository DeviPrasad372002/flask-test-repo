...
def _get_article_payload(result):
    # If serializer returns {"article": {...}} or {"articles": [...]}, normalize
    if isinstance(result, _exc_lookup("Exception", Exception)) and "article" in result and isinstance(result["article"], dict):
        return result["article"]
    return result

def _get_articles_list(result):
    if isinstance(result, _exc_lookup("Exception", Exception)):
        if "articles" in result and isinstance(result["articles"], list):
            return result["articles"], result.get("articlesCount", len(result["articles"]))
        # some serializers might return {"articles": [...]} or {"items": [...]}
        for key in ("items", "results"):
            if key in result and isinstance(result[key], list):
                return result[key], len(result[key])
    if isinstance(result, _exc_lookup("Exception", Exception)):
        return result, len(result)
    # unknown format
    return [], 0

def _get_comment_payload(result):
    if isinstance(result, _exc_lookup("Exception", Exception)) and "comment" in result and isinstance(result["comment"], dict):
        return result["comment"]
    return result

def _sig_accepts(obj, *args_names):
    sig = inspect.signature(obj)
    params = list(sig.parameters.keys())
    return all(name in params for name in args_names)

def _call_with_fallback(fn, *possible_args):
    sig = inspect.signature(fn)
    params = list(sig.parameters.keys())
    # Attempt to call with the longest matching prefix of possible_args
    for n in range(len(possible_args), -1, -1):
        args = possible_args[:n]
        if len(args) <= len(params):
            try:
                return fn(*args)
            except TypeError:
                continue
    # last resort: try calling with zero args
    return fn()

def test_dump_article_and_dump_articles_structure():
    # Import target functions inside test
    import conduit.articles.serializers as serializers

    # Build deterministic dummy objects matching expected serializer access patterns
    class DummyAuthor:
        ...
        # Some serializers might access as dict-like
        def to_dict(self):
    ...
    class DummyArticle:
        def __init__(self, slug, title, description, body, tags, author_username):
            self.slug = slug
            self.title = title
            self.description = description
            self.body = body
            # support both tagList and tags attribute access
            self.tagList = list(tags)
            self.tags = list(tags)
            # support createdAt/updatedAt strings or created_at datetimes
            self.created_at = now
            self.updated_at = now
            self.createdAt = now_iso
            self.updatedAt = now_iso
            self.author = DummyAuthor(author_username, bio="bio:"+author_username, image="img:"+author_username, following=False)
            # methods that some serializers call
    ...
    a1 = DummyArticle("slug-one", "Title One", "Desc 1", "Body 1", ["py", "test"], "alice")
    a2 = DummyArticle("slug-two", "Title Two", "Desc 2", "Body 2", ["go"], "bob")

    # Call dump_article on single article
    dump_article = serializers.dump_article
    # Some implementations accept (article, current_user) or (article,)
    result_single = _call_with_fallback(dump_article, a1, None)
    payload = _get_article_payload(result_single)
    assert isinstance(payload, _exc_lookup("Exception", Exception))
    # Required keys commonly present in conduit article responses
    for key in ("slug", "title", "description", "body", "tagList", "createdAt", "updatedAt", "favorited", "favoritesCount", "author"):
        assert key in payload, f"missing {key} in dumped article payload"
    # Validate values come from our dummy article
    assert payload["slug"] == a1.slug
    assert payload["title"] == a1.title
    # tagList should be list and contain our tags
    assert isinstance(payload["tagList"], (list, tuple))
    assert set(payload["tagList"]) >= set(a1.tagList)
    # author should be nested structure with username
    assert isinstance(payload["author"], dict)
    assert payload["author"].get("username") == a1.author.username

    # Call dump_articles for multiple articles
    dump_articles = serializers.dump_articles
    result_many = _call_with_fallback(dump_articles, [a1, a2])
    articles_list, count = _get_articles_list(result_many)
    assert len(articles_list) == 2
    assert count == 2

def test_dump_comment_structure():
    import conduit.articles.serializers as serializers
    ...
    class DummyComment:
        def __init__(self, comment_id, body, author_username):
            self.id = comment_id
            self.body = body
            self.created_at = now
            self.updated_at = now
            self.createdAt = now_iso
            self.updatedAt = now_iso
            self.author = DummyAuthor(author_username)
        # support to satisfy different serializer expectations
        def to_dict(self):
            return {"id": self.id, "body": self.body}

    comment = DummyComment(123, "This is a comment.", "charlie")

    dump_comment = serializers.dump_comment
    result = _call_with_fallback(dump_comment, comment, None)
    payload = _get_comment_payload(result)
    assert isinstance(payload, _exc_lookup("Exception", Exception))
    # Common comment fields
    for key in ("id", "createdAt", "updatedAt", "body", "author"):
        assert key in payload, f"missing {key} in dumped comment payload"
    assert payload["id"] == comment.id
    assert payload["body"] == comment.body
    assert isinstance(payload["author"], dict)
    assert payload["author"].get("username") == comment.author.username
