...
def test_article_model_surface_and_constructor_errors():
    # Import targets inside the test to keep tests self-contained
    from conduit.articles import models as article_models

    # Ensure Article class exists
    assert hasattr(article_models, "Article")
    Article = article_models.Article
    assert inspect.isclass(Article)

    # Expected API surface on Article model
    expected_attrs = [
        "favourite", "unfavourite", "is_favourite",
        "favoritesCount", "favorited", "add_tag", "remove_tag",
    ]
    for attr in expected_attrs:
        assert hasattr(Article, attr), f"Article missing attribute {attr}"
        assert callable(getattr(Article, attr)), f"Article.{attr} should be callable"

    # Constructing Article without required args should raise TypeError (interface check)
    with pytest.raises(_exc_lookup("TypeError", Exception)):
        Article()  # likely requires init args; just verify it errors deterministically

def test_views_and_serializers_surface_and_signature_contracts():
    # Import view functions and serializers inside test
    from conduit.articles import views as article_views
    from conduit.articles import serializers as article_serializers

    # Views we expect to exist (black-box surface)
    view_funcs = [
        "get_article", "delete_article", "favorite_an_article", "unfavorite_an_article",
        "articles_feed", "get_tags", "get_comments", "make_comment_on_article",
        "delete_comment_on_article",
    ]
    for name in view_funcs:
        assert hasattr(article_views, name), f"views missing {name}"
        func = getattr(article_views, name)
        assert callable(func), f"{name} should be callable"
        # Calling without required context/args should raise TypeError for most view callables
        with pytest.raises(_exc_lookup("TypeError", Exception)):
            func()  # ensure signature enforces inputs

    # Serializers / helper constructors expected to exist
    serializer_objs = [
        "make_article", "dump_article", "dump_articles",
        "make_comment", "dump_comment",
        "CommentSchema", "ArticleSchema", "ArticleSchemas", "TagSchema", "Meta",
    ]
    for name in serializer_objs:
        assert hasattr(article_serializers, name), f"serializers missing {name}"
        obj = getattr(article_serializers, name)
        # Classes or callables should be usable for instantiation or direct call:
        if inspect.isclass(obj):
            # Instantiating without args may raise or succeed depending on implementation;
            # if it raises, we expect a TypeError; otherwise ensure instance is created.
            try:
                instance = obj()
                assert instance is not None
            except Exception as e:
                assert isinstance(e, _exc_lookup("TypeError", Exception)) or isinstance(e, _exc_lookup("Exception", Exception))
        else:
            # callable functions - calling with no args should normally raise TypeError
            with pytest.raises(_exc_lookup("TypeError", Exception)):
                obj()
