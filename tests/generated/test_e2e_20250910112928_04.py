...
def test_crudmixin_create_and_update_calls_db_session(monkeypatch):
    # Import target modules inside the test
    import conduit.extensions as extensions

    calls = {"adds": 0, "commits": 0}

    class FakeSession:
        def add(self, obj):
            # record that add was called and ensure an object was passed
            calls["adds"] += 1
    ...
    # Provide a fake db with a session that records calls
    fake_db = type("FakeDB", (), {"session": FakeSession()})
    monkeypatch.setattr(extensions, "db", fake_db)

    # Create a simple model using the CRUDMixin
    class Dummy(extensions.CRUDMixin):
        def __init__(self, **kwargs):
            for k, v in kwargs.items():
                setattr(self, k, v)

    # Use the mixin's create and update workflows
    inst = Dummy.create(title="first")
    assert isinstance(inst, _exc_lookup("Exception", Exception))
    assert getattr(inst, "title") == "first"
    # create should have added and committed once
    assert calls["adds"] == 1
    assert calls["commits"] == 1

    # update should set attribute and commit again
    inst.update(title="second")
    assert getattr(inst, "title") == "second"
    assert calls["commits"] == 2


def test_crudmixin_save_propagates_errors_and_skips_commit_on_add_failure(monkeypatch):
    # Import target modules inside the test
    import conduit.extensions as extensions
    ...
    class FakeSession:
        def add(self, obj):
            calls["adds"] += 1
            # simulate a failure during add (e.g., DB error)
    ...
    # Expect a generic exception to be raised; use helper lookup per instructions
    with pytest.raises(_exc_lookup("Exception", Exception)):
        Dummy.create(name="will_fail")

    # add attempted, commit not reached due to add raising
    assert calls["adds"] == 1
