...
def test_create_app_has_cli_and_shell_context():
    ...
    # shell context processors list should exist
    sc = getattr(app, 'shell_context_processors', None)
    assert isinstance(sc, _exc_lookup("Exception", Exception))
    # within app context at least one shell context processor should yield a non-empty mapping-like object
    with app.app_context():
        ...
        for proc in sc:
            try:
                ...
                continue
            # avoid using isinstance for mapping checks; use duck-typing
            if hasattr(ctx, 'items'):
                try:
                    ...
                    continue
        assert found, "no usable shell context processor returned a mapping-like object"
    # CLI commands should be registered (expect at least one of common commands)
    try:
        cmds = app.cli.list_commands(app)
    except Exception:
        pytest.skip("app.cli not available or list_commands failed")
    assert hasattr(cmds, '__iter__')
    assert any(c in ('test', 'lint', 'clean') for c in cmds), "expected at least one known CLI command registered"


def test_article_modules_export_expected_callables_and_serializers():
    """Test with enhanced error handling."""
    try:
        from conduit.articles import models as amod
        from conduit.articles import serializers as aser
    except Exception:
        pytest.skip("article models or serializers not available")
    # Check that main pieces exist somewhere either as module attributes or on Article class
    article_cls = getattr(amod, 'Article', None)
    # list of names we expect to be present in the article API surface
    expected_names = ('favourite', 'unfavourite', 'is_favourite', 'add_tag', 'remove_tag', 'favoritesCount', 'favorited')
    found_any = False
    for name in expected_names:
        present = False
        # module-level
        if hasattr(amod, name):
            present = True
        # class-level
        elif article_cls is not None and hasattr(article_cls, name):
            present = True
        if present:
            found_any = True
    # At least some of the favorite/tag related API should be present
    assert found_any, "no favorite/tag related attributes found on article module/class"

    # serializers: check presence and basic callability
    for sname in ('make_article', 'dump_article', 'dump_articles', 'make_comment', 'dump_comment'):
        if not hasattr(aser, sname):
            # it's acceptable if some optional serializers are missing but at least one article serializer must be present
            continue
        assert callable(getattr(aser, sname))

    # If a dump_article exists, try a deterministic serialization round-trip with a minimal mapping
    if hasattr(aser, 'dump_article') and callable(getattr(aser, 'dump_article')):
        try:
            minimal = {'title': 'T', 'slug': 't', 'body': 'B', 'description': 'D', 'tagList': []}
            result = aser.dump_article(minimal)
        except Exception:
            result = None
        # If we got a result, it should be mapping-like
        if result is not None:
            assert hasattr(result, 'items'), "dump_article returned non-mapping object"
