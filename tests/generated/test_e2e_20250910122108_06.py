...
def test_create_app_uses_test_config():
    ...
    try:
        ...
        pytest.skip("create_app or TestConfig unavailable")
    # Attempt to create app using the TestConfig class/object
    try:
        app = create_app(TestConfig)
    except TypeError:
        # Some factories accept a string key instead; try a common alternative
        try:
            app = create_app('testing')
        except Exception as e:
            pytest.skip(f"create_app failed with TestConfig and fallback: {e}")
    # Basic, deterministic checks that indicate TestConfig was applied
    assert getattr(app, "testing", app.config.get("TESTING", False)) is True
    # Ensure app has an import name relating to the package
    assert "conduit" in getattr(app, "import_name", app.name)

def test_profile_serialization_and_invalidusage_behavior():
    """Test with enhanced error handling."""
    try:
        ...
        pytest.skip("Profile models/serializers or InvalidUsage unavailable")

    def _exc_lookup(name, fallback):
        try:
            import conduit.exceptions as excmod
            return getattr(excmod, name, fallback)
        except Exception:
            return fallback

    # Try several reasonable constructor signatures for UserProfile until one works
    profile = None
    candidate_kw = [
        {"username": "alice"},
        {"username": "alice", "bio": "hello"},
        {"username": "alice", "bio": "hello", "image": None, "following": False},
    ]
    for kw in candidate_kw:
        try:
            profile = UserProfile(**kw)
            break
        except TypeError:
            try:
                profile = UserProfile(kw.get("username"))
                break
            except Exception:
                continue
        except Exception:
            continue
    if profile is None:
        pytest.skip("Could not instantiate UserProfile with common signatures")

    # Serialize with the ProfileSchema
    try:
        schema = ProfileSchema()
        serialized = schema.dump(profile)
    except Exception:
        pytest.skip("ProfileSchema.dump failed")
    # Expect the username to be preserved in serialization
    assert serialized.get("username") == "alice"

    # Construct an InvalidUsage instance in a robust way
    invalid = None
    try:
        invalid = InvalidUsage("something went wrong", status_code=400)
    except TypeError:
        try:
            invalid = InvalidUsage("something went wrong")
        except Exception:
            pytest.skip("Unable to construct InvalidUsage")
    except Exception:
        pytest.skip("Unable to construct InvalidUsage")

    # Ensure it's an instance of the expected exception type via _exc_lookup
    assert isinstance(invalid, _exc_lookup("Exception", Exception))

    # Try to retrieve a JSON representation if available, otherwise fall back to str()
    if hasattr(invalid, "to_json"):
        try:
            payload = invalid.to_json()
        except Exception:
            pytest.skip("InvalidUsage.to_json raised unexpectedly")
        # Accept either dict-like JSON or string containing the message
        if isinstance(payload, _exc_lookup("Exception", Exception)):
            assert any(k in payload for k in ("message", "error", "errors"))
        else:
            assert "something went wrong" in str(payload)
    else:
        assert "something went wrong" in str(invalid)
