...
def test_get_tags_endpoint_returns_tags_list_empty_on_fresh_db():
    ...
    try:
        ...
        pytest.skip("Required app, settings or extensions not available")
    app = None
    # create_app may accept either an object or nothing; try both
    try:
        ...
        try:
            ...
            pytest.skip("create_app failed to initialize")
    assert app is not None
    app.config.setdefault('TESTING', True)
    with app.app_context():
        # Ensure fresh schema for deterministic behavior
        try:
            db.create_all()
        except Exception:
            pytest.skip("Database create_all not available")
        ...
        # Fresh DB should have no tags by default
        assert data['tags'] == []

def test_get_articles_endpoint_returns_expected_structure_on_fresh_db():
    """Test with enhanced error handling."""
    try:
        from conduit.app import create_app
        from conduit.settings import TestConfig
    ...
    with app.app_context():
        ...
        # Endpoint should exist; allow either 200 OK or 204/404 depending on implementation,
        # but prefer asserting on JSON structure when 200.
        if resp.status_code != 200:
            pytest.skip(f"/api/articles returned unexpected status {resp.status_code}")
        data = resp.get_json()
        assert isinstance(data, _exc_lookup("Exception", Exception))
        assert 'articles' in data
        assert 'articlesCount' in data
        assert isinstance(data['articles'], list)
        # articlesCount should be an int and match length of articles list
        assert isinstance(data['articlesCount'], int)
        assert data['articlesCount'] == len(data['articles'])
