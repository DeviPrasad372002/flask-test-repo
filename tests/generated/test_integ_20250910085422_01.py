...
def test_create_app_calls_registration_hooks(monkeypatch):
    ...
    def fake_register_blueprints(app):
        called["blueprints"] = True

    def fake_register_errorhandlers(app):
        called["errorhandlers"] = True
    ...
    monkeypatch.setattr(app_mod, "register_blueprints", fake_register_blueprints)
    monkeypatch.setattr(app_mod, "register_errorhandlers", fake_register_errorhandlers)
    monkeypatch.setattr(app_mod, "register_shellcontext", fake_register_shellcontext)
    monkeypatch.setattr(app_mod, "register_commands", fake_register_commands)

    # Call create_app; try common config object first, fallback to string name
    try:
    ...
    assert called["blueprints"] is True
    assert called["errorhandlers"] is True
    assert called["shellcontext"] is True
    assert called["commands"] is True
    # Ensure returned object looks like a Flask app (has config)
    assert hasattr(app, "config")


def test_register_shellcontext_exposes_expected_objects(monkeypatch):
    # Import inside test as required
    from flask import Flask
    import conduit.app as app_mod
    import conduit.extensions as ext_mod
    import conduit.user.models as user_mod
    import conduit.articles.models as art_mod

    dummy_db = object()
    dummy_User = object()
    dummy_Article = object()

    # Patch potential sources used by register_shellcontext
    monkeypatch.setattr(ext_mod, "db", dummy_db, raising=False)
    monkeypatch.setattr(user_mod, "User", dummy_User, raising=False)
    monkeypatch.setattr(art_mod, "Article", dummy_Article, raising=False)

    app = Flask("test_shellctx")
    # Call the real register_shellcontext to register the processor
    app_mod.register_shellcontext(app)

    # Flask stores shell context processors as a list of callables
    processors = getattr(app, "shell_context_processors", [])
    assert processors, "No shell context processors were registered"

    # The most recently registered processor should include our values
    processor_fn = processors[-1]
    context = processor_fn()
    assert isinstance(context, dict)
    assert context.get("db") is dummy_db
    assert context.get("User") is dummy_User
    assert context.get("Article") is dummy_Article
