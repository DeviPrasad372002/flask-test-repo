...
def test_create_app_calls_registers(monkeypatch):
    ...
    def fake_register_extensions(app):
        calls.append('register_extensions_called')

    def fake_register_shellcontext(app):
        calls.append('register_shellcontext_called')

    def fake_register_commands(app):
        calls.append('register_commands_called')

    # Provide a lightweight fake Flask so create_app doesn't need real Flask internals
    class DummyApp:
        def __init__(self, *a, **k):
            self.config = {}
            self.import_name = a[0] if a else 'dummy'

    def FakeFlask(*a, **k):
        return DummyApp(*a, **k)

    monkeypatch.setattr(appmod, 'register_extensions', fake_register_extensions)
    monkeypatch.setattr(appmod, 'register_shellcontext', fake_register_shellcontext)
    monkeypatch.setattr(appmod, 'register_commands', fake_register_commands)
    monkeypatch.setattr(appmod, 'Flask', FakeFlask)

    # Call create_app; if it expects a config name, supply one. If not, the FakeFlask will handle it.
    try:
        app = appmod.create_app('testing')
    except TypeError:
        app = appmod.create_app()

    # Basic assertions that our fake registration functions were invoked and an app-like object returned
    assert hasattr(app, 'config'), "create_app did not return an app-like object with 'config'"
    assert 'register_extensions_called' in calls
    assert 'register_shellcontext_called' in calls
    assert 'register_commands_called' in calls

def test_crud_mixin_and_save_use_db_session(monkeypatch):
    # Import target inside test
    import conduit.extensions as ext

    # Create a fake DB session that records calls
    class FakeSession:
        def __init__(self):
            self.added = []
    ...
    # Patch the module-level db used by ext to our fake
    monkeypatch.setattr(ext, 'db', fake_db)

    # Define a minimal model that inherits CRUDMixin
    class Model(ext.CRUDMixin):
        def __init__(self):
            # allow introspection of instance
            self._marker = object()

    m = Model()

    # Test instance save via the mixin; should add and commit via our fake session
    returned = m.save()
    assert returned is m
    assert fake_session.added, "Expected the instance to be added to the session"
    assert fake_session.added[0] is m
    assert fake_session.committed is True

    # Reset session state and test module-level save function if present
    fake_session.added.clear()
    fake_session.committed = False

    # If ext.save is callable, exercise it; otherwise skip gracefully
    save_func = getattr(ext, 'save', None)
    if callable(save_func):
        returned2 = save_func(m)
        assert returned2 is m
        assert fake_session.added and fake_session.added[0] is m
        assert fake_session.committed is True
    else:
        # ensure that absence of save function is explicit (fail the test in that case)
        raise _exc_lookup('NameError', Exception)("Expected 'save' function in conduit.extensions")
