...
def _exc_lookup(name, default=Exception):
    try:
        import conduit.exceptions as ex_mod
        return getattr(ex_mod, name)
    except Exception:
        return default
...
def test_save_commits_and_returns_object(monkeypatch):
    ...
    # Create a fake session to capture calls
    ...
    # Replace the db object used by extensions with a fake that has session
    fake_db = types.SimpleNamespace(session=FakeSession())
    monkeypatch.setattr(extensions, "db", fake_db, raising=False)

    # Reload save from module to ensure it uses the monkeypatched db
    ...
    assert calls["added"] is obj
    assert calls["committed"] is True

def test_shell_context_includes_core_objects():
    # Import inside test
    import conduit.app as appmod

    # shell_context should return a mapping of useful objects for the shell
    ctx = appmod.shell_context()

    assert isinstance(ctx, dict)
    # Expect common keys to be present; values should be non-None
    for key in ("db", "User", "Article", "Profile"):
        assert key in ctx
        assert ctx[key] is not None

def test_dump_article_serializer_produces_expected_structure():
    # Import inside test
    from types import SimpleNamespace
    import conduit.articles.serializers as serializers

    # Construct a lightweight article-like object
    ...
    # Call serializer dump function
    dumped = serializers.dump_article(article, current_user=None)
    ...
    art = dumped["article"]
    # Basic fields should round-trip
    assert art.get("slug") == "test-article"
    assert art.get("title") == "Test Article"
    assert art.get("description") == "Desc"
    assert art.get("body") == "Body content"
    # Tag list should be preserved as a list
    assert isinstance(art.get("tagList"), list)
    assert set(art.get("tagList")) == {"python", "testing"}
    # Author sub-object should be a dict-like structure with username
    author_dump = art.get("author")
    assert isinstance(author_dump, dict)
    assert author_dump.get("username") == "alice"
