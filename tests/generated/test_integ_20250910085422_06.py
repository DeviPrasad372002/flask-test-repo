...
def test_create_app_registers_extensions_and_uses_testconfig(monkeypatch):
    import flask
    # Import inside test as required
    import conduit.app as app_module
    # Stub out register_extensions so create_app doesn't initialize real extensions
    def fake_register_extensions(app, config=None):
        # simulate side effect of registering extensions
        setattr(app, "extensions_registered", True)
    monkeypatch.setattr(app_module, "register_extensions", fake_register_extensions, raising=True)

    # Call create_app with the TestConfig path
    app = app_module.create_app("conduit.settings.TestConfig")

    assert isinstance(app, flask.Flask)
    # our fake side effect
    assert getattr(app, "extensions_registered", False) is True
    # TestConfig in typical setups sets TESTING True; ensure the config value is present and truthy
    assert app.config.get("TESTING", False) is True


def test_crudmixin_save_and_update_uses_db_session(monkeypatch):
    # Import inside test
    from types import SimpleNamespace
    import conduit.extensions as extensions_module

    # Create a dummy session to capture calls
    class DummySession:
        def __init__(self):
            self.added = []
    ...
    # Patch the db used by CRUDMixin
    monkeypatch.setattr(extensions_module, "db", dummy_db, raising=True)

    # Dynamically create a model that uses CRUDMixin
    class DummyModel(extensions_module.CRUDMixin):
        def __init__(self, **kwargs):
            # allow arbitrary attributes
            for k, v in kwargs.items():
                setattr(self, k, v)

    # Instance and test save()
    m = DummyModel(name="original")
    # ensure no prior added
    assert dummy_session.added == []
    m.save()
    # After save, object should have been added and committed
    assert m in dummy_session.added
    assert dummy_session.committed is True

    # Test update changes attributes and triggers a save (which would call commit)
    # Reset commit flag to observe call again
    dummy_session.committed = False
    m.update(name="changed", extra=123)
    assert getattr(m, "name") == "changed"
    assert getattr(m, "extra") == 123
    # update normally results in a save/commit cycle; ensure commit happened
    assert dummy_session.committed is True


def test_profile_schema_serializes_userprofile_without_db(monkeypatch):
    # Import inside test
    import conduit.profile.models as profile_models
    import conduit.profile.serializers as profile_serializers

    # Create a lightweight UserProfile-like object. Avoid DB by not invoking model constructor that touches DB.
    # If UserProfile is a class with __init__ that may rely on DB, create a simple stand-in object with expected attributes.
    # We'll try to use the real class if it can be constructed without DB side effects, otherwise fallback to SimpleNamespace.
    try:
        ProfileClass = profile_models.UserProfile
        # attempt to construct with no args; if it fails, fallback is handled by except
        profile = ProfileClass()
    except Exception:
        from types import SimpleNamespace
        profile = SimpleNamespace()
    # Populate commonly expected profile fields
    setattr(profile, "username", "alice")
    setattr(profile, "bio", "Example bio")
    setattr(profile, "image", None)
    # Some schemas expect 'following' property; provide it
    setattr(profile, "following", False)

    schema = profile_serializers.ProfileSchema()
    result = schema.dump(profile)

    # The schema should produce a dict-like result
    assert isinstance(result, dict)
    # At minimum ensure the serialized output contains the username somewhere
    serialized_text = str(result)
    assert "alice" in serialized_text


def test_settings_classes_relationships_and_attributes():
    # Import inside test
    import conduit.settings as settings_module

    Config = settings_module.Config
    TestConfig = settings_module.TestConfig
    ProdConfig = settings_module.ProdConfig
    DevConfig = settings_module.DevConfig

    # Ensure subclass relationships
    assert issubclass(TestConfig, Config)
    assert issubclass(ProdConfig, Config)
    assert issubclass(DevConfig, Config)

    # Check that TestConfig has a TESTING flag enabled and ProdConfig tends to disable DEBUG
    # Use getattr with defaults in case attribute names differ slightly
    assert getattr(TestConfig, "TESTING", True) is True
    # ProdConfig should not be in testing mode by default
    assert getattr(ProdConfig, "TESTING", False) is False or getattr(ProdConfig, "DEBUG", False) is False
