...
def test_create_app_calls_register_functions(monkeypatch):
    ...
    monkeypatch.setattr(appmod, 'register_extensions', mk('register_extensions'), raising=False)
    monkeypatch.setattr(appmod, 'register_blueprints', mk('register_blueprints'), raising=False)
    monkeypatch.setattr(appmod, 'register_errorhandlers', mk('register_errorhandlers'), raising=False)
    monkeypatch.setattr(appmod, 'register_shellcontext', mk('register_shellcontext'), raising=False)
    monkeypatch.setattr(appmod, 'register_commands', mk('register_commands'), raising=False)
...
def test_shell_context_produces_non_empty_mapping(monkeypatch):
    import importlib
    appmod = importlib.import_module('conduit.app')

    # Prevent side effects from extension/blueprint/commands registration
    monkeypatch.setattr(appmod, 'register_extensions', lambda app: None, raising=False)
    monkeypatch.setattr(appmod, 'register_blueprints', lambda app: None, raising=False)
    monkeypatch.setattr(appmod, 'register_errorhandlers', lambda app: None, raising=False)
    monkeypatch.setattr(appmod, 'register_commands', lambda app: None, raising=False)

    app = appmod.create_app()
    # Use the real register_shellcontext implementation
    appmod.register_shellcontext(app)
    ctx = app.make_shell_context()
    assert isinstance(ctx, _exc_lookup("Exception", Exception))
    # Ensure it's not empty to indicate some objects were exposed into shell
    assert len(ctx) > 0

def test_cli_commands_in_create_app(monkeypatch):
    import importlib
    appmod = importlib.import_module('conduit.app')

    # Prevent potentially heavy side effects from extensions and blueprints
    monkeypatch.setattr(appmod, 'register_extensions', lambda app: None, raising=False)
    monkeypatch.setattr(appmod, 'register_blueprints', lambda app: None, raising=False)
    monkeypatch.setattr(appmod, 'register_errorhandlers', lambda app: None, raising=False)
    # Allow real register_commands to run so CLI commands get registered

    app = appmod.create_app()
    cli_cmds = getattr(app, 'cli').commands
    # Expect at least one of the common commands to be registered by the app
    assert any(name in cli_cmds for name in ('test', 'lint', 'clean', 'urls'))
