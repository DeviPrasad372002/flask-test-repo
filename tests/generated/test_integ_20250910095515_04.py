...
def test_create_app_registers_components(monkeypatch):
    ...
    def make_spy(key):
        def spy(app):
            called[key] = app
            # Some register functions may return values; return None to be safe
            return None
        return spy

    monkeypatch.setattr(appmod, "register_extensions", make_spy("register_extensions"))
    monkeypatch.setattr(appmod, "register_blueprints", make_spy("register_blueprints"))
    monkeypatch.setattr(appmod, "register_errorhandlers", make_spy("register_errorhandlers"))
    monkeypatch.setattr(appmod, "register_shellcontext", make_spy("register_shellcontext"))
    monkeypatch.setattr(appmod, "register_commands", make_spy("register_commands"))

    app = create_app(TestConfig)
    # Ensure an app instance returned
    assert app is not None
    # Ensure all spies were invoked with an app instance
    expected_keys = {"register_extensions", "register_blueprints", "register_errorhandlers", "register_shellcontext", "register_commands"}
    assert expected_keys.issubset(set(called.keys()))
    for v in called.values():
        # ensure the passed object looks like a Flask app (has name attribute and import_name)
        assert hasattr(v, "import_name") and hasattr(v, "config")

def test_create_calls_save_and_returns_instance(monkeypatch):
    # Test that conduit.extensions.create instantiates the model and delegates to save
    from conduit.extensions import create as ext_create
    import conduit.extensions as extmod

    saved = []

    def fake_save(instance):
        # emulate save assigning an id and returning the instance
        setattr(instance, "id", 12345)
        saved.append(instance)
        return instance

    monkeypatch.setattr(extmod, "save", fake_save)

    class DummyModel:
        def __init__(self, **kwargs):
            self.__dict__.update(kwargs)

    inst = ext_create(DummyModel, title="Integration Test", published=True)
    # returned value should be the instance and saved via save()
    assert inst is not None
    assert saved and saved[0] is inst
    assert getattr(inst, "title") == "Integration Test"
    assert getattr(inst, "published") is True
    assert getattr(inst, "id") == 12345

def test_save_uses_db_session_add_and_commit(monkeypatch):
    # Test that conduit.extensions.save uses db.session.add and commit
    import conduit.extensions as extmod
    ...
    class DummySession:
        ...
        def refresh(self, obj):
            events.append(("refresh", obj))

    fake_db = types.SimpleNamespace(session=DummySession())
    monkeypatch.setattr(extmod, "db", fake_db)

    class SimpleObj:
        pass

    obj = SimpleObj()
    result = extmod.save(obj)

    # save should return the same object
    assert result is obj

    # ensure add and commit were called on session
    assert any(ev[0] == "add" and ev[1] is obj for ev in events), "db.session.add was not called with the object"
    assert any(ev[0] == "commit" for ev in events), "db.session.commit was not called"
