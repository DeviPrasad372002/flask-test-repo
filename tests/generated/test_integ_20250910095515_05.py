...
def _exc_lookup(name, base=Exception):
    try:
        ...
        return getattr(_excmod, name, base)
    except Exception:
        return base

def test_update_calls_save_and_sets_attributes(monkeypatch):
    # Integration test: ensure conduit.extensions.update applies attributes and triggers save
    ext = pytest.importorskip('conduit.extensions')

    class DummyModel:
        def __init__(self):
            # start with no attributes set
            self._saved = False

    obj = DummyModel()

    called = {"save": False, "args": None, "kwargs": None}

    def fake_save(target, *a, **k):
        # record call and do a noop
        called["save"] = True
        called["args"] = a
        called["kwargs"] = k
        # if the model has a flag, set it
        try:
            target._saved = True
        except Exception:
            pass
    ...
    # Monkeypatch save function on the extensions module (add or replace)
    monkeypatch.setattr(ext, "save", fake_save, raising=False)

    data = {"title": "New Title", "body": "New Body"}

    if not hasattr(ext, "update"):
        pytest.skip("conduit.extensions.update not available")

    # Try calling update with a data dict or as kwargs depending on implementation.
    tried = False
    success = False
    for attempt in (("dict",), ("kwargs",),):
        try:
            tried = True
            if attempt[0] == "dict":
                # common signature: update(model, data)
                res = ext.update(obj, data)
            else:
                # alternative: update(model, **data)
                res = ext.update(obj, **data)
            success = True
            break
        except TypeError:
            ...
            # If update intentionally raises a domain error, re-raise as test failure unless it's a known custom exception
            exc_cls = _exc_lookup(e.__class__.__name__, Exception)
    ...
    if not tried:
        pytest.skip("Could not invoke conduit.extensions.update in any supported form")

    if not success:
        pytest.skip("conduit.extensions.update exists but could not be invoked with common signatures")

    # After update, expect attributes to be set on the object (update implementations normally set attributes via setattr)
    # Accept either direct attribute assignment or return value containing the updated object
    updated_obj = None
    if hasattr(obj, "title") or hasattr(obj, "body"):
        updated_obj = obj
    elif res is not None:
        updated_obj = res

    assert updated_obj is not None, "update did not return or modify the object as expected"

    # Check attributes if present
    if hasattr(updated_obj, "title"):
        assert getattr(updated_obj, "title") == "New Title"
    if hasattr(updated_obj, "body"):
        assert getattr(updated_obj, "body") == "New Body"

    # Ensure save was called (our fake_save should have been invoked by update)
    assert called["save"], "conduit.extensions.update did not call conduit.extensions.save"

def test_shell_context_exposes_core_objects():
    # Integration test: ensure the app shell_context provides commonly expected objects
    appmod = pytest.importorskip("conduit.app")

    if not hasattr(appmod, "shell_context"):
        pytest.skip("conduit.app.shell_context not present")

    ctx = appmod.shell_context()

    # Expect a mapping-like return from shell_context
    assert isinstance(ctx, _exc_lookup("Exception", Exception)), "shell_context() should return a dict-like mapping"

    # Look for several common keys that an application usually exposes to shell context
    # Be permissive: require at least one of the commonly expected entries
    expected_keys = {"db", "User", "Article", "Comment", "Profile", "Tag"}
    found = expected_keys.intersection(set(ctx.keys()))
    assert found, f"shell_context returned keys {set(ctx.keys())}; expected at least one of {expected_keys}"

    # If db is present, ensure it exposes basic session/Model attributes without touching external resources
    if "db" in ctx:
        db = ctx["db"]
        # check for common SQLAlchemy attributes in a safe manner
        has_session = any(hasattr(db, attr) for attr in ("session", "engine", "Model"))
        assert has_session, "db object in shell_context does not appear to be a SQLAlchemy-like object"
