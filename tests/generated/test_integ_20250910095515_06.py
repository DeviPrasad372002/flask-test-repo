...
def test_config_classes_inheritance():
    # Import targets inside test
    from conduit.settings import Config, DevConfig, ProdConfig, TestConfig

    # Basic integration checks: subclass relationships
    assert issubclass(DevConfig, Config)
    assert issubclass(ProdConfig, Config)
    assert issubclass(TestConfig, Config)

    # Ensure classes are distinct objects
    assert DevConfig is not ProdConfig
    assert ProdConfig is not TestConfig
    assert DevConfig is not TestConfig

def test_crudmixin_save_and_update_commits(monkeypatch):
    # Import CRUDMixin inside test
    ...
    class DummySession:
        ...
        def commit(self):
            self.committed += 1
        def delete(self, obj):
            # record deletion as a negative marker
    ...
    dummy_db = DummyDB()
    # Patch the db used by CRUDMixin implementation
    monkeypatch.setattr(ext_mod, "db", dummy_db, raising=False)

    # Create a model class that uses CRUDMixin
    class Model(CRUDMixin):
        def __init__(self, value=None):
            self.value = value

    m = Model(value=1)
    # Call save and expect the dummy session to record the add and commit
    saved = m.save()
    assert saved is m
    assert dummy_db.session.added and dummy_db.session.added[-1] is m
    assert dummy_db.session.committed >= 1

    # Reset counters and test update with kwargs - expects attribute change and a commit
    dummy_db.session.added.clear()
    dummy_db.session.committed = 0
    updated = m.update(foo="bar", value=42)
    # update typically returns the instance
    assert updated is m
    # attributes should be applied
    assert getattr(m, "foo", None) == "bar"
    assert m.value == 42
    # ensure save side-effect happened (add + commit)
    assert dummy_db.session.added and dummy_db.session.added[-1] is m
    assert dummy_db.session.committed >= 1

def test_invalidusage_can_be_raised_and_toserializable():
    # Import InvalidUsage via dynamic lookup helper inside test
    import importlib
    exceptions_mod = importlib.import_module("conduit.exceptions")

    def _exc_lookup(name, default):
        return getattr(exceptions_mod, name, default)

    InvalidUsage = _exc_lookup("Exception", Exception)

    # Creating and raising the custom exception should be catchable via pytest.raises lookup
    with pytest.raises(_exc_lookup("Exception", Exception)):
        # many InvalidUsage implementations accept (message, status_code, payload)
        # create with a message; if signature differs, it should still be an Exception subclass
        raise InvalidUsage("test problem occurred")

    # If the exception defines to_json / to_dict style API, it should produce a serializable value
    inst = InvalidUsage("another problem")
    # Try both common names
    to_json = getattr(inst, "to_json", None) or getattr(inst, "to_dict", None) or getattr(inst, "toString", None)
    if to_json is None:
        # If no serializing method, ensure instance is an Exception
        assert isinstance(inst, _exc_lookup("Exception", Exception))
    else:
        out = to_json()
        assert isinstance(out, (dict, str))

def test_userprofile_and_profileschema_dump(monkeypatch):
    # Import targets inside test
    from conduit.profile.models import UserProfile
    from conduit.profile.serializers import ProfileSchema

    # Create a simple UserProfile instance. Try to pass commonly used attributes.
    # If constructor signature differs, create via minimal kwargs or by setting attributes afterward.
    try:
        profile = UserProfile(username="alice", bio="bio text", image=None)
    except TypeError:
        # fallback: construct empty then set attributes
        profile = UserProfile()
        for k, v in {"username": "alice", "bio": "bio text", "image": None}.items():
            try:
                setattr(profile, k, v)
            except Exception:
    ...
    # Marshmallow typically returns a dict; ensure we get a mapping-like result
    assert isinstance(dumped, (dict,))
    # If username is included in serialization, it should match
    if "username" in dumped:
        assert dumped["username"] == "alice"
    # If 'bio' included, verify type
    if "bio" in dumped:
        assert isinstance(dumped["bio"], (str, type(None)))
