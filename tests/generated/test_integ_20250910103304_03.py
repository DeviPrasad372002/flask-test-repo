...
def _call_with_signature(func, *provided):
    sig = inspect.signature(func)
    params = list(sig.parameters)
    args = []
    for i, name in enumerate(params):
        if i < len(provided):
            args.append(provided[i])
        else:
...
def test_execute_tool_invokes_subprocess(monkeypatch, tmp_path):
    # import target inside test
    from conduit.commands import execute_tool

    called = {}

    def stub_run(*a, **k):
        called.setdefault('run', []).append((a, k))
        # mimic subprocess.run returning CompletedProcess
        cp = types.SimpleNamespace(returncode=0)
        return cp

    def stub_check_call(*a, **k):
        called.setdefault('check_call', []).append((a, k))
        return 0

    def stub_call(*a, **k):
        called.setdefault('call', []).append((a, k))
        return 0

    # Patch common subprocess entrypoints used by execute_tool
    monkeypatch.setattr(subprocess, 'run', stub_run)
    monkeypatch.setattr(subprocess, 'check_call', stub_check_call)
    monkeypatch.setattr(subprocess, 'call', stub_call)

    # Call execute_tool with typical args; be resilient to signature
    _call_with_signature(execute_tool, 'echo', ['hello'], {'ENV_TEST': '1'}, str(tmp_path))

    assert called, "Expected at least one subprocess entrypoint to be invoked"
    # ensure the recorded call contains the tool name or args list
    any_ok = False
    for entries in called.values():
        for args, kwargs in entries:
            ...
            for item in args:
                try:
                    ...
                    flat.append(item)
            if 'echo' in flat or any('echo' == x for x in flat):
                any_ok = True
    assert any_ok, "execute_tool did not seem to invoke subprocess with expected command"


def test_execute_tool_passes_env_and_cwd(monkeypatch, tmp_path):
    from conduit.commands import execute_tool

    recorded = {'calls': []}

    def stub_runner(*a, **k):
        recorded['calls'].append((a, k))
        return types.SimpleNamespace(returncode=0)

    monkeypatch.setattr(subprocess, 'run', stub_runner)
    monkeypatch.setattr(subprocess, 'check_call', stub_runner)
    monkeypatch.setattr(subprocess, 'call', stub_runner)

    env = {'FOO': 'BAR'}
    cwd = str(tmp_path / "workdir")
    # create dir to mimic cwd existence
    (tmp_path / "workdir").mkdir()

    # Call with env and cwd if signature allows; be flexible
    _call_with_signature(execute_tool, 'echo', ['x'], env, cwd)

    assert recorded['calls'], "No subprocess invocation recorded"
    # ensure at least one call carried our env or cwd in kwargs
    found_env_or_cwd = False
    for args, kwargs in recorded['calls']:
        if kwargs.get('env') == env or kwargs.get('cwd') == cwd:
            found_env_or_cwd = True
        # sometimes env/cwd may be passed positionally in older code; inspect args
        for item in args:
            if item == env or item == cwd:
                found_env_or_cwd = True
    assert found_env_or_cwd, "execute_tool did not propagate env or cwd to subprocess call"


def test_urls_uses_glob_and_returns_paths(monkeypatch):
    from conduit.commands import urls
    import glob as real_glob

    fake_files = [
        '/project/conduit/app.py',
        '/project/conduit/commands.py',
        '/project/conduit/some/module.py',
    ]

    def fake_glob(pattern, recursive=True):
        # ignore pattern and return our fake list
        return list(fake_files)

    monkeypatch.setattr(real_glob, 'glob', fake_glob)

    result = urls()
    # result should be an iterable of strings derived from file paths
    assert result, "urls() returned empty result"
    # Check that each basename from fake_files appears in the output
    basenames = {p.split('/')[-1] for p in fake_files}
    output_joined = " ".join(map(str, result))
    for name in basenames:
        assert name in output_joined, f"Expected {name} to appear in urls() output"
