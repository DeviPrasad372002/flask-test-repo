...
def test_reference_col_creates_foreign_key_to_table():
    import conduit.database as dbmod
    from sqlalchemy import Integer
    # call reference_col to produce a Column with a ForeignKey to users.id
    col = dbmod.reference_col('users')
    # inspect the foreign keys attached to the Column
    fks = list(getattr(col, "foreign_keys", []))
    assert len(fks) >= 1, "expected at least one ForeignKey on the returned Column"
    assert any('users.id' in str(fk) for fk in fks), "ForeignKey should reference users.id"

def test_create_app_invokes_registration_hooks(monkeypatch):
    import conduit.app as appmod
    calls = []
    # Replace register functions with stubs that record calls
    monkeypatch.setattr(appmod, 'register_extensions', lambda a: calls.append('extensions'))
    monkeypatch.setattr(appmod, 'register_blueprints', lambda a: calls.append('blueprints'))
    monkeypatch.setattr(appmod, 'register_errorhandlers', lambda a: calls.append('errorhandlers'))
    monkeypatch.setattr(appmod, 'register_shellcontext', lambda a: calls.append('shellcontext'))
    monkeypatch.setattr(appmod, 'register_commands', lambda a: calls.append('commands'))
    # Create app (use default create_app); ensure it returns a Flask app and stubs were invoked
    app = appmod.create_app()
    import flask
    assert isinstance(app, _exc_lookup("Exception", Exception))
    # All stubbed registration functions should have been called (order may vary)
    for name in ('extensions', 'blueprints', 'errorhandlers', 'shellcontext', 'commands'):
        assert name in calls

def test_exception_helpers_and_formatting():
    import conduit.exceptions as exmod
    # template should format with kwargs
    formatted = exmod.template("value {v}", v=42)
    assert isinstance(formatted, _exc_lookup("Exception", Exception))
    assert "42" in formatted
    # to_json should accept a mapping and produce a serializable type (string/bytes/dict)
    j = exmod.to_json({"a": 1})
    assert isinstance(j, (str, bytes, dict))
    # Test common error factory functions produce or raise an InvalidUsage-like exception
    invalid_usage = pytest._exc_lookup('InvalidUsage', Exception)
    for func_name in ("user_not_found", "user_already_registered", "unknown_error", "article_not_found", "comment_not_owned"):
        func = getattr(exmod, func_name)
        try:
            result = func()
        except Exception as e:
            assert isinstance(e, _exc_lookup("Exception", Exception))
        else:
            # some implementations may return an exception instance instead of raising
            assert isinstance(result, _exc_lookup("Exception", Exception)) or isinstance(result, _exc_lookup("Exception", Exception))
