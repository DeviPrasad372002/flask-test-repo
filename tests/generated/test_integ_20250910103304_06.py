...
def test_create_app_registers_components(monkeypatch):
    ...
    # Replace the register helper functions with stubs that record calls
    monkeypatch.setattr(appmod, "register_extensions", lambda app: calls.append("extensions"))
    monkeypatch.setattr(appmod, "register_blueprints", lambda app: calls.append("blueprints"))
    monkeypatch.setattr(appmod, "register_errorhandlers", lambda app: calls.append("errorhandlers"))
    monkeypatch.setattr(appmod, "register_shellcontext", lambda app: calls.append("shellcontext"))
    monkeypatch.setattr(appmod, "register_commands", lambda app: calls.append("commands"))

    # create_app should call our stubbed register functions
    app = appmod.create_app("testing")

    # Basic assertions about the created Flask app and that our stubs ran
    from flask import Flask
    assert isinstance(app, _exc_lookup("Exception", Exception))
    # Ensure the config corresponds to TestConfig
    assert app.config.get("TESTING", False) == getattr(TestConfig, "TESTING", True)
    # Ensure all registration stubs were invoked
    assert set(calls) >= {"extensions", "blueprints", "errorhandlers", "shellcontext", "commands"}


def test_crud_mixin_save_update_create(monkeypatch):
    # Import inside test
    from conduit import extensions

    # Create a dummy DB/session to capture actions
    class DummySession:
        def __init__(self):
            self.added = []
            self.committed = 0
    ...
    dummy_db = DummyDB()
    # Monkeypatch the db used by CRUDMixin module to our dummy
    monkeypatch.setattr(extensions, "db", dummy_db)

    # Compose a minimal model using the CRUDMixin
    class Model(extensions.CRUDMixin):
        def __init__(self, **kwargs):
            # assign passed kwargs as attributes for update/create behavior
            for k, v in kwargs.items():
                setattr(self, k, v)

    # Test save() adds and commits
    m = Model()
    # ensure no prior adds
    assert dummy_db.session.added == []
    m.save()
    assert dummy_db.session.added and dummy_db.session.added[-1] is m
    assert dummy_db.session.committed >= 1

    # Test update() modifies attributes and persists
    before_commits = dummy_db.session.committed
    returned = m.update({"a": 10, "b": "x"})
    # update may return self; ensure attributes set
    assert getattr(m, "a", None) == 10
    assert getattr(m, "b", None) == "x"
    # update should trigger a save/commit (commit count increased)
    assert dummy_db.session.committed >= before_commits

    # Test create() as a classmethod creates, persists, and returns an instance
    before_len = len(dummy_db.session.added)
    created = Model.create(c=3, d="z")
    # created should be an instance
    assert isinstance(created, _exc_lookup("Exception", Exception))
    assert getattr(created, "c", None) == 3
    assert getattr(created, "d", None) == "z"
    # create should have added the instance
    assert len(dummy_db.session.added) >= before_len + 1


def test_invalidusage_object_properties_and_to_json_like(monkeypatch):
    # Import inside test
    from conduit import exceptions

    # Construct an InvalidUsage instance and validate attributes and serialization method behavior
    iu = exceptions.InvalidUsage("something went wrong", status_code=422, payload={"detail": "x"})
    # The exception should carry provided message and status
    assert "something went wrong" in str(iu)
    assert getattr(iu, "status_code", None) == 422
    # If to_json exists, it should return a dict or JSON-serializable structure
    to_json = getattr(iu, "to_json", None)
    if callable(to_json):
        result = to_json()
        assert isinstance(result, _exc_lookup("Exception", Exception))
        # Ensure payload or message is present in the JSON structure in some form
        flat_values = set()
        def extract_vals(obj):
            if isinstance(obj, _exc_lookup("Exception", Exception)):
                ...
                flat_values.add(obj)
        extract_vals(result)
        assert any("something went wrong" == v or "x" == v or "detail" == v or "payload" == v for v in flat_values) or len(flat_values) > 0
    else:
        # If no to_json method, at minimum ensure we have payload attribute accessible
        assert getattr(iu, "payload", None) == {"detail": "x"}
