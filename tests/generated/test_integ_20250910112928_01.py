...
def _exc_lookup(name, default):
    # dynamic lookup for exception/class by name across loaded modules
...
def test_register_extensions_calls_init_app(monkeypatch):
    ...
    import importlib
    ext_module = importlib.import_module('conduit.extensions')
    register_module = importlib.import_module('conduit.app')
    ...
    class FakeExt:
        ...
        def init_app(self, application):
            # record that init_app was called and that the passed app is the same
            self.init_called = True
            called.append((self.name, application is app))

    # Common extension names typically used; set them on conduit.extensions
    names = ['db', 'migrate', 'bcrypt', 'cache', 'cors', 'jwt', 'apispec']
    for n in names:
        fake = FakeExt(n)
        monkeypatch.setattr(ext_module, n, fake, raising=False)

    # Call the function under test
    register_module.register_extensions(app)

    # Ensure that at least some of the fakes were initialized
    assert len(called) >= 1
    # All registered fakes that exist on the module should have had init_app called
    for n in names:
        ext = getattr(ext_module, n, None)
        if ext is not None:
            assert getattr(ext, 'init_called', False) is True
...
def test_invalidusage_to_json_and_isinstance():
    ...
    # Lookup InvalidUsage class via dynamic lookup helper
    InvalidUsage = None
    if hasattr(exc_mod, 'InvalidUsage'):
        InvalidUsage = getattr(exc_mod, 'InvalidUsage')

    # If it's not present directly on module, try global lookup
    if InvalidUsage is None:
        InvalidUsage = _exc_lookup('InvalidUsage', None)

    assert InvalidUsage is not None, "InvalidUsage exception/class not found in project"

    # Construct an instance; constructor signature may vary, try common forms
    try:
        err = InvalidUsage("boom")
    except TypeError:
        # maybe requires message and status_code
        try:
            err = InvalidUsage("boom", status_code=400)
        except Exception:
            # fallback to calling without args
            err = InvalidUsage()

    # to_json may exist on instance or class
    to_json = getattr(err, 'to_json', None)
    if to_json is None:
        # maybe module has a function to_json
        to_json = getattr(exc_mod, 'to_json', None)

    # Ensure isinstance recognition via _exc_lookup
    exc_cls = _exc_lookup('InvalidUsage', Exception)
    assert isinstance(err, _exc_lookup("Exception", Exception))

    # If to_json exists, ensure it returns a dict-like mapping
    if to_json is not None:
        data = to_json() if callable(to_json) and getattr(to_json, '__self__', None) is not None else to_json(err) if callable(to_json) else None
        assert isinstance(data, _exc_lookup("Exception", Exception))

def test_favourite_and_unfavourite_flow(monkeypatch):
    # Import models module inside test
    import importlib
    models = importlib.import_module('conduit.articles.models')

    # Prepare dummy article and user to interact with model-level functions or methods.
    class DummyUser:
    ...
    class DummyArticle:
        def __init__(self):
            # Many implementations store users in a list-like attribute named favorited or favourited
            self.favorited = []
            self._saved = False
        def save(self):
            self._saved = True
        # Provide is_favourite method if the module expects to call it on instance
        def is_favourite(self, user):
            # compare by id if available
            for u in self.favorited:
                if getattr(u, 'id', None) == getattr(user, 'id', None):
                    return True
            return False
        @property
    ...
    # If module provides top-level functions 'favourite' and 'unfavourite', use them.
    used_top_level = False
    if hasattr(models, 'favourite') and hasattr(models, 'unfavourite'):
        used_top_level = True
        fav_fn = getattr(models, 'favourite')
        unfav_fn = getattr(models, 'unfavourite')
        # Call favourite
        fav_fn(article, user)
        assert article.is_favourite(user) is True
        assert article.favoritesCount >= 1
        # Call unfavourite
        unfav_fn(article, user)
        assert article.is_favourite(user) in (False, False)
    else:
        # Otherwise, try methods on an Article class if present
        Article = getattr(models, 'Article', None)
        if Article is None:
            pytest.skip("No favourite/unfavourite functions or Article class available in conduit.articles.models")
        # Create instance in a DB-independent way: try to instantiate with no args
        try:
            a = Article()
        except Exception:
            # Attempt to create a dummy instance by bypassing __init__
            a = object.__new__(Article)
            # set attributes expected by methods on class
            try:
                setattr(a, 'favorited', [])
            except Exception:
                pass
            # provide save if missing
            if not hasattr(a, 'save'):
                setattr(a, 'save', lambda: None)

        # Ensure methods exist
        assert any(hasattr(a, name) for name in ('favourite', 'favorite', 'unfavourite', 'unfavorite'))
        # Try favorite method names
        if hasattr(a, 'favourite'):
            getattr(a, 'favourite')(user)
            assert getattr(a, 'is_favourite')(user) is True
            getattr(a, 'unfavourite')(user)
            assert getattr(a, 'is_favourite')(user) is False
        elif hasattr(a, 'favorite'):
            getattr(a, 'favorite')(user)
            assert getattr(a, 'is_favourite')(user) is True
            getattr(a, 'unfavorite')(user)
            assert getattr(a, 'is_favourite')(user) is False
        else:
            pytest.skip("Article class does not expose favorite/unfavorite style methods")
