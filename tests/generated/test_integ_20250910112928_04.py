...
def _exc_lookup(name, default):
    # Try common modules where exceptions may live
...
def test_get_by_id_uses_model_query_get(monkeypatch):
    ...
    class DummyQuery:
        def __init__(self):
            self.last_seen = None
        def get(self, ident):
            # record and return a simple sentinel object
            self.last_seen = ident
            return {"found": True, "id": ident}

    class FakeModel:
        query = DummyQuery()

    result = get_by_id(FakeModel, 123)
    assert isinstance(result, _exc_lookup("Exception", Exception))
    assert result.get("found") is True
    assert result.get("id") == 123
    # ensure the underlying query.get was called with the same id
    assert FakeModel.query.last_seen == 123

def test_execute_tool_calls_subprocess_check_call(monkeypatch):
    # Import target inside test
    ...
    # Monkeypatch subprocess.check_call
    monkeypatch.setattr(subprocess, "check_call", fake_check_call)

    # Should not raise; just ensure our fake was invoked and saw the tool name
    execute_tool("mytool-name")
    assert 'cmd' in captured
    assert "mytool-name" in str(captured['cmd'])

def test_execute_tool_on_failure_exits(monkeypatch):
    from conduit.commands import execute_tool

    def fake_check_call_fail(cmd, *args, **kwargs):
        # Simulate a failing external tool invocation
        raise subprocess.CalledProcessError(returncode=2, cmd=cmd)

    monkeypatch.setattr(subprocess, "check_call", fake_check_call_fail)

    with pytest.raises(_exc_lookup("Exception", Exception)):
        execute_tool("will-fail")
