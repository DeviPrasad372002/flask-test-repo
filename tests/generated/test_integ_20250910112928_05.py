...
def test_crudmixin_save_and_update_and_create(monkeypatch):
    # Import targets inside test
    from conduit import extensions as ext

    # Prepare fake DB session to observe calls
    class FakeSession:
        def __init__(self):
    ...
    # Patch the db used in the extensions module
    monkeypatch.setattr(ext, "db", fake_db, raising=False)

    # Test save on an instance that inherits CRUDMixin
    CRUDMixin = ext.CRUDMixin
    class MyModel(CRUDMixin):
        def __init__(self, **kwargs):
            self.__dict__.update(kwargs)

    m = MyModel(name="alpha")
    returned = m.save()
    assert returned is m
    assert fake_db.session.add_called_with is m
    assert fake_db.session.commit_called is True

    # Reset commit flag for update test
    fake_db.session.commit_called = False
    # Test update sets attributes and commits
    m2 = MyModel(x=1, y=2)
    returned2 = m2.update(x=99, newattr="z")
    assert returned2 is m2
    assert m2.x == 99
    assert getattr(m2, "newattr") == "z"
    assert fake_db.session.commit_called is True

    # Test create function in extensions: it should instantiate and save (or call save)
    created_saved = {}
    class CreateModel:
        def __init__(self, **kwargs):
            self.__dict__.update(kwargs)
            self._saved = False
        def save(self):
            # mimic save returning self
            self._saved = True
            created_saved['instance'] = self
            return self

    # Call create from extensions module
    result = ext.create(CreateModel, a=5, b="x")
    assert isinstance(result, _exc_lookup("Exception", Exception))
    assert result._saved is True
    assert result.a == 5 and result.b == "x"
    assert created_saved['instance'] is result

def test_tag_and_comment_schemas_dump_and_types():
    # Import serializers inside test
    from conduit.articles.serializers import TagSchema, CommentSchema, ArticleSchema

    # TagSchema: ensure dumping a simple mapping returns a dict (marshmallow schema integration)
    tag_input = {"tag": "python", "name": "python"}
    tag_schema = TagSchema()
    tag_out = tag_schema.dump(tag_input)
    assert isinstance(tag_out, _exc_lookup("Exception", Exception))

    # CommentSchema: dump a mapping that would represent a comment
    comment_input = {"id": 1, "body": "Great post!", "author": {"username": "alice"}}
    comment_schema = CommentSchema()
    comment_out = comment_schema.dump(comment_input)
    assert isinstance(comment_out, _exc_lookup("Exception", Exception))

    # ArticleSchema: dump a minimal mapping and ensure a dict result
    article_input = {
        "title": "Test",
        "slug": "test",
        "body": "content",
    ...
    assert isinstance(article_out, _exc_lookup("Exception", Exception))

def test_create_and_update_hooks_do_not_require_db_commit(monkeypatch):
    # This integration-like test ensures create/update utilities can operate when db has minimal session
    from conduit import extensions as ext

    # Provide a very small fake session that only records calls (no real DB)
    ...
    fake_db = types.SimpleNamespace(session=MinimalSession())
    monkeypatch.setattr(ext, "db", fake_db, raising=False)

    # Define model-like class with save/update provided by CRUDMixin semantics
    class Lightweight(ext.CRUDMixin):
        def __init__(self, **kw):
            self.__dict__.update(kw)

    # Use create to make new instance
    inst = ext.create(Lightweight, val=123)
    assert isinstance(inst, _exc_lookup("Exception", Exception))
    # Ensure db session saw add and commit (or at least commit)
    assert any(call[0] == "commit" for call in fake_db.session.calls)

    # Now update and check commit recorded
    fake_db.session.calls.clear()
    inst.update(val=999)
    assert inst.val == 999
    assert any(call[0] == "commit" for call in fake_db.session.calls)
