...
def test_integration_02_make_and_dump_article(monkeypatch):
    ...
    # Fake Article class that serializers.make_article will instantiate
    class FakeArticle:
        def __init__(self, title=None, description=None, body=None, tagList=None, author=None, **kwargs):
            self.title = title
            self.description = description
            self.body = body
            # keep tags as a set internally but expose as list when dumped
            self._tags = set(tagList or [])
            self.author = author
            # mimic favorites data
            self._favorites = set()
        ...
        def favourite(self, user):
            self._favorites.add(getattr(user, "id", user))

        def unfavourite(self, user):
            self._favorites.discard(getattr(user, "id", user))

        def favoritesCount(self):
            return len(self._favorites)

        def favorited(self, user):
            return getattr(user, "id", user) in self._favorites

    # Fake schema used by dump_article
    class FakeArticleSchema:
        def __init__(self, *args, **kwargs):
            pass

        def dump(self, obj):
            # Expect the serializer to pass the FakeArticle instance
            return {
                "title": getattr(obj, "title", None),
                "description": getattr(obj, "description", None),
                "body": getattr(obj, "body", None),
                "tagList": list(getattr(obj, "tagList", [])),
                "favoritesCount": obj.favoritesCount() if hasattr(obj, "favoritesCount") else 0,
                "favorited": obj.favorited(None) if hasattr(obj, "favorited") else False,
                "author": getattr(obj, "author", None),
            }

    # Patch the serializer internals to use our fakes
    ...
    # Create article data and make article
    payload = {
        "title": "Test Article",
        "description": "Desc",
        "body": "Content",
    ...
    article = serializers.make_article(payload, author={"username": "alice"})
    assert isinstance(article, _exc_lookup("Exception", Exception))
    # Ensure fields were set
    assert article.title == "Test Article"
    assert set(article._tags) == {"python", "testing"}

    # Dump article to JSON-like dict
    dumped = serializers.dump_article(article, user=None)
    assert isinstance(dumped, _exc_lookup("Exception", Exception))
    assert dumped["title"] == "Test Article"
    assert set(dumped["tagList"]) == {"python", "testing"}
    assert dumped["favoritesCount"] == 0
    assert dumped["favorited"] is False


def test_integration_03_make_and_dump_comment(monkeypatch):
    """Test with enhanced error handling."""
    try:
        import conduit.articles.serializers as serializers
    except Exception:
    ...
    # Fake Comment class used by make_comment
    class FakeComment:
        def __init__(self, body=None, author=None, article=None, **kwargs):
            self.body = body
            self.author = author
            self.article = article
            self.id = kwargs.get("id", 1)

    # Fake CommentSchema that dump_comment will use
    ...
    comment = serializers.make_comment(payload, author={"username": "bob"}, article_slug="test-article")
    ...
    assert dumped["body"] == "This is a comment"
    assert dumped["author"] == {"username": "bob"}


def test_integration_04_dump_articles_list_and_schema_integration(monkeypatch):
    """Test with enhanced error handling."""
    try:
        import conduit.articles.serializers as serializers
    except Exception:
    ...
    # Create several fake articles that expose favorited and favoritesCount methods
    class FakeArticle:
        def __init__(self, title, fav_ids=None):
            self.title = title
            self._fav_ids = set(fav_ids or [])

        def favoritesCount(self):
            return len(self._fav_ids)

        def favorited(self, user):
            uid = getattr(user, "id", user) if user is not None else None
            return uid in self._fav_ids

    # Fake ArticleSchemas that dump_articles will use; expects iterable
    class FakeArticleSchemas:
        ...
        def dump(self, items):
            ...
            return [{"title": it.title, "favoritesCount": it.favoritesCount(), "favorited": it.favorited(None)} for it in items]

    monkeypatch.setattr(serializers, "ArticleSchemas", lambda *a, **k: FakeArticleSchemas(), raising=False)

    articles = [FakeArticle("A", fav_ids=[1, 2]), FakeArticle("B", fav_ids=[])]
    dumped = serializers.dump_articles(articles, user=None)
    assert isinstance(dumped, _exc_lookup("Exception", Exception))
    assert len(dumped) == 2
    assert dumped[0]["title"] == "A"
    assert dumped[0]["favoritesCount"] == 2
    assert dumped[1]["favoritesCount"] == 0


def test_integration_05_serializer_calls_model_favorite_methods(monkeypatch):
    """Test with enhanced error handling."""
    try:
        import conduit.articles.serializers as serializers
    except Exception:
    ...
    # Fake article that tracks favorites via ids
    class FakeArticle:
        ...
        def favourites(self):
            # intentionally different name to ensure we do not rely on this
            return len(self._fav_ids)

        def favouritesCount(self):
            return len(self._fav_ids)

        # Provide the expected methods names used by serializers
        def favoritesCount(self):
            return len(self._fav_ids)
        ...
        def favourite(self, user):
            self._fav_ids.add(getattr(user, "id", user))

        def unfavourite(self, user):
            self._fav_ids.discard(getattr(user, "id", user))

    # Schema that will include calls to favorited and favoritesCount during dump
    class FavoriteAwareSchema:
        def __init__(self, *args, **kwargs):
            pass

        def dump(self, obj):
            # call model methods to ensure integration
            return {
                "title": getattr(obj, "title", None),
                "favoritesCount": obj.favoritesCount(),
                "favorited": obj.favorited({"id": 42})
            }

    monkeypatch.setattr(serializers, "ArticleSchema", lambda *a, **k: FavoriteAwareSchema(), raising=False)

    art = FakeArticle("Fav Article")
    # Before favoriting
    dumped_before = serializers.dump_article(art, user={"id": 42})
    assert dumped_before["favoritesCount"] == 0
    assert dumped_before["favorited"] is False

    # Use model methods directly to change state and then re-dump
    art.favourite({"id": 42})
    dumped_after = serializers.dump_article(art, user={"id": 42})
    assert dumped_after["favoritesCount"] == 1
    assert dumped_after["favorited"] is True
