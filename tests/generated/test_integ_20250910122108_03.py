...
def test_execute_tool_invokes_subprocess_run(monkeypatch):
    ...
    def fake_run(cmd, *args, **kwargs):
        # record the command and return a dummy CompletedProcess-like object
        called['cmd'] = cmd
        class Dummy:
            returncode = 0
        return Dummy()
    # commands module likely imports subprocess at top-level
    if hasattr(commands, 'subprocess'):
        monkeypatch.setattr(commands.subprocess, 'run', fake_run)
    else:
        # fallback: patch standard subprocess if module used via import inside function
        import subprocess as _subp
        monkeypatch.setattr(_subp, 'run', fake_run)
    # call execute_tool with a simple command list
    # ensure the function exists
    if not hasattr(commands, 'execute_tool'):
        pytest.skip("execute_tool not available in conduit.commands")
    result = commands.execute_tool(['echo', 'hello'])
    # verify our fake was called
    assert 'cmd' in called and isinstance(called['cmd'], (list, tuple))
    assert list(called['cmd'])[0] in ('echo',)

def test_cli_commands_call_execute_tool(monkeypatch):
    """Test with enhanced error handling."""
    try:
        import conduit.commands as commands
    except Exception:
        pytest.skip("conduit.commands not importable")
    if not any(hasattr(commands, name) for name in ('test', 'lint', 'clean')):
        pytest.skip("expected CLI command functions not present")
    seen = []
    def fake_execute_tool(cmd):
        seen.append(list(cmd))
        return 0
    # Patch execute_tool in the module so CLI commands use our fake
    monkeypatch.setattr(commands, 'execute_tool', fake_execute_tool)
    # call available CLI functions if present
    if hasattr(commands, 'test'):
        try:
            commands.test()
        except Exception:
            # allow commands to raise; we just want to ensure execute_tool was invoked
            pass
    if hasattr(commands, 'lint'):
        try:
            commands.lint()
        except Exception:
            pass
    if hasattr(commands, 'clean'):
        try:
            commands.clean()
        except Exception:
            pass
    # ensure execute_tool was invoked at least once and we saw common commands keywords
    assert len(seen) >= 1
    # try to detect that typical tools are requested by at least one command invocation
    flattened = " ".join(" ".join(s) for s in seen).lower()
    assert any(keyword in flattened for keyword in ('pytest', 'flake8', 'git', 'clean'))

def test_crudmixin_save_uses_extensions_save(monkeypatch):
    """Test with enhanced error handling."""
    try:
        import conduit.extensions as extensions
    except Exception:
        pytest.skip("conduit.extensions not importable")
    if not hasattr(extensions, 'CRUDMixin'):
        pytest.skip("CRUDMixin not available in conduit.extensions")
    called = {}
    def fake_save(instance):
        called['instance'] = instance
        return True
    # Monkeypatch the save function in the extensions module
    monkeypatch.setattr(extensions, 'save', fake_save)
    # create a lightweight dummy that inherits CRUDMixin
    CRUDMixin = extensions.CRUDMixin
    class Dummy(CRUDMixin):
        def __init__(self):
            self._marker = True
    d = Dummy()
    # call the mixin's save method which should call the patched extensions.save
    # Some implementations name the mixin method 'save' too; call it if present
    if hasattr(d, 'save'):
        result = d.save()
        # ensure our fake was called with the instance
        assert called.get('instance') is d
    else:
        pytest.skip("CRUDMixin does not provide a save method to call")
