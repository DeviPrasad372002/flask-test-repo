...
def test_save_uses_db_session(monkeypatch):
    ...
    # Monkeypatch the db attribute used by the save function
    monkeypatch.setattr(ext_mod, 'db', fake_db, raising=False)

    # Prepare a dummy object to save
    dummy = object()

    # Call save and verify interactions with the fake session
    try:
        ext_mod.save(dummy)
    except Exception as e:
        # If save raises, still fail the test unless it's an import-time mismatch
        raise

    assert fake_db.session.added and fake_db.session.added[0] is dummy
    assert getattr(fake_db.session, 'committed') is True

def test_create_app_registers_extension_and_shellcontext(monkeypatch):
    """Test with enhanced error handling."""
    try:
    ...
    # Replace registration helper functions with spies
    def _spy_register_extensions(app):
        calls.append('register_extensions_called')
    def _spy_register_shellcontext(app):
        calls.append('register_shellcontext_called')
    def _spy_register_blueprints(app):
        calls.append('register_blueprints_called')
    def _spy_register_errorhandlers(app):
        calls.append('register_errorhandlers_called')
    # Monkeypatch attributes on the module; if they don't exist, set them anyway
    monkeypatch.setattr(app_mod, 'register_extensions', _spy_register_extensions, raising=False)
    monkeypatch.setattr(app_mod, 'register_shellcontext', _spy_register_shellcontext, raising=False)
    monkeypatch.setattr(app_mod, 'register_blueprints', _spy_register_blueprints, raising=False)
    monkeypatch.setattr(app_mod, 'register_errorhandlers', _spy_register_errorhandlers, raising=False)

    create_app = getattr(app_mod, 'create_app', None)
    if create_app is None:
        pytest.skip("create_app not found in conduit.app")

    # Try calling create_app with different likely signatures to be robust
    app = None
    call_attempts = [
        (),                     # no args
        ('testing',),           # config name
        ({'TESTING': True},),   # config dict
    ]
    for args in call_attempts:
        try:
            # Use inspect to ensure we don't pass unexpected args
            sig = inspect.signature(create_app)
            params = sig.parameters
            # Only call with args length <= params count or if varargs exist
            if any(p.kind == inspect.Parameter.VAR_POSITIONAL for p in params.values()) or len(args) <= len(params):
                app = create_app(*args)
                break
        except TypeError:
            continue
        except Exception:
            # If create_app raised due to environment, still consider this a test failure
            raise

    if app is None:
        pytest.skip("Unable to call create_app with any common signature")

    # After successful creation, our spies should have been invoked
    assert 'register_extensions_called' in calls
    assert 'register_shellcontext_called' in calls
    assert 'register_blueprints_called' in calls or isinstance(app, _exc_lookup("Exception", Exception))  # blueprints may be optional
    # errorhandlers registration may be optional; ensure at least extensions and shellcontext were invoked
