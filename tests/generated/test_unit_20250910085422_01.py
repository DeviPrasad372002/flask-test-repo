...
def test_article_repr_and_basic_init():
    # Import inside test
    try:
        from conduit.articles.models import Article
    except Exception:
        pytest.skip("conduit.articles.models.Article not available")
    # Instantiate (many models allow blank construction)
    art = Article()
    # set some simple attributes that __repr__ / __init__ might reference
    setattr(art, "title", "Test Title")
    setattr(art, "slug", "test-title")
...
def _get_favorites_count(article):
    # support either method or attribute
    cnt = getattr(article, "favoritesCount", None)
    if cnt is None:
        cnt = getattr(article, "favorites_count", None)
    if callable(cnt):
        return cnt()
    return cnt

def _is_fav(article, user):
    # support is_favourite method or favorited collection
    isfav = getattr(article, "is_favourite", None)
    if callable(isfav):
        return bool(isfav(user))
    favs = getattr(article, "favorited", None) or getattr(article, "favorites", None)
    if favs is None:
        return False
    try:
        return user in favs
    except Exception:
        # maybe favors stores ids
        uid = getattr(user, "id", None)
        return uid in favs

def test_article_favourite_and_unfavourite_cycle():
    try:
        from conduit.articles.models import Article
    except Exception:
        pytest.skip("conduit.articles.models.Article not available")
    if not hasattr(Article, "favourite") or not hasattr(Article, "unfavourite"):
        pytest.skip("Article.favourite/unfavourite not implemented")
    art = Article()
    ...
    start_cnt = _get_favorites_count(art)
    if start_cnt is None:
        start_cnt = 0
    assert isinstance(start_cnt, int) or start_cnt == 0
    # favourite the article
    art.favourite(u)
    assert _is_fav(art, u) is True
    cnt_after = _get_favorites_count(art)
    assert isinstance(cnt_after, int)
    assert cnt_after >= 1
    # unfavourite returns to not-favourited
    art.unfavourite(u)
    assert _is_fav(art, u) is False
    cnt_final = _get_favorites_count(art)
    assert isinstance(cnt_final, int)
    assert cnt_final <= cnt_after

def test_favourite_idempotent_when_called_multiple_times():
    try:
        from conduit.articles.models import Article
    except Exception:
        pytest.skip("conduit.articles.models.Article not available")
    ...
    class U:
        def __init__(self, id): self.id = id
    u = U(42)
    # call favourite twice
    art.favourite(u)
    art.favourite(u)
    cnt = _get_favorites_count(art)
    assert isinstance(cnt, int)
    # idempotent: count should not increase on second call (i.e., >=1)
    assert cnt >= 1
    # now unfavourite once and ensure removed
    art.unfavourite(u)
    assert _is_fav(art, u) is False
