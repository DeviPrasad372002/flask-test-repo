...
def test_dump_article_wraps_schema_dump(monkeypatch):
    import conduit.articles.serializers as serializers

    created = {}
    class DummyArticleSchema:
        def __init__(self, *args, **kwargs):
            created['inst'] = self
        def dump(self, obj):
            # record what was passed and return a predictable dict
            self.last_dumped = obj
            return {'title': 'Serialized Title'}

    monkeypatch.setattr(serializers, 'ArticleSchema', DummyArticleSchema)

    # simple plain object to represent an article
    ArticleLike = type('ArticleLike', (), {})
    article = ArticleLike()
    article.title = 'Original Title'

    result = serializers.dump_article(article)

    assert result == {'article': {'title': 'Serialized Title'}}
    assert created['inst'].last_dumped is article

def test_dump_articles_returns_list_and_count(monkeypatch):
    import conduit.articles.serializers as serializers
    ...
    class DummyArticleSchema:
        def __init__(self, *a, **k):
            pass
        def dump(self, obj):
            # incorporate object's marker so we can distinguish items
            return {'slug': getattr(obj, 'slug', 's')}
    ...
    result = serializers.dump_articles([a1, a2])

    assert result == {'articles': [{'slug': 'one'}, {'slug': 'two'}], 'articlesCount': 2}

def test_make_article_uses_schema_load_and_returns_object(monkeypatch):
    import conduit.articles.serializers as serializers
    ...
    class DummyArticleSchema:
        def __init__(self, *a, **k):
            recorded['init_kwargs'] = (a, k)
        def load(self, data):
            # record received payload and return sentinel object
            recorded['loaded'] = data
            return sentinel

    monkeypatch.setattr(serializers, 'ArticleSchema', DummyArticleSchema)

    payload = {'article': {'title': 'Payload Title'}}
    result = serializers.make_article(payload)

    assert result is sentinel
    assert recorded['loaded'] == payload['article']

def test_make_and_dump_comment_roundtrip(monkeypatch):
    import conduit.articles.serializers as serializers

    # prepare dummy schema classes for comment creation and dumping
    created = {}
    class DummyCommentSchema:
        def __init__(self, *a, **k):
            created.setdefault('instances', []).append(self)
        def load(self, data):
            # simulate creation object
            obj = type('CommentObj', (), {})()
            obj.body = data.get('body')
            obj.id = 123
            return obj
        def dump(self, obj):
            return {'id': getattr(obj, 'id', None), 'body': getattr(obj, 'body', None)}

    monkeypatch.setattr(serializers, 'CommentSchema', DummyCommentSchema)

    payload = {'comment': {'body': 'Hello'}}
    created_obj = serializers.make_comment(payload)
    assert hasattr(created_obj, 'body') and created_obj.body == 'Hello'
    # now dump the comment
    dumped = serializers.dump_comment(created_obj)
    assert dumped == {'comment': {'id': 123, 'body': 'Hello'}}
