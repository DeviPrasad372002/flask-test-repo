...
def test_exception_factory_helpers_produce_invalidusage_and_serializable_payload():
    import importlib
    exc = importlib.import_module('conduit.exceptions')
    helpers = ('user_not_found', 'user_already_registered', 'unknown_error', 'article_not_found', 'comment_not_owned')
    InvalidUsage = _exc_lookup('InvalidUsage', Exception)
    for h in helpers:
        assert hasattr(exc, h), f"expected exception helper {h} to exist"
        factory = getattr(exc, h)
        e = factory()
        assert isinstance(e, InvalidUsage)
        # to_json should accept the exception instance and produce a mapping or (mapping, status)
        assert hasattr(exc, 'to_json'), "to_json function missing in conduit.exceptions"
        out = exc.to_json(e)
        ...
        assert isinstance(payload, dict), "expected payload to be a dict"
        # find message text from exception
        msg = getattr(e, 'message', None) or getattr(e, 'msg', None) or (e.args[0] if getattr(e, 'args', None) else None)
        assert msg is not None
        # ensure payload contains the message somewhere in its values
        values = [v for v in payload.values()]
        assert any((isinstance(v, str) and msg in v) or (v == msg) for v in values)

def test_template_and_to_json_consistency_for_custom_status():
    import importlib
    exc = importlib.import_module('conduit.exceptions')
    InvalidUsage = _exc_lookup('InvalidUsage', Exception)
    assert hasattr(exc, 'template'), "template function missing in conduit.exceptions"
    e = exc.template('it broke', 418)
    assert isinstance(e, InvalidUsage)
    out = exc.to_json(e)
    if isinstance(out, tuple):
        payload, status = out
    ...
    msg = getattr(e, 'message', None) or getattr(e, 'msg', None) or (e.args[0] if getattr(e, 'args', None) else None)
    assert any((isinstance(v, str) and msg in v) or (v == msg) for v in payload.values())

def test_execute_tool_invokes_subprocess_variant(monkeypatch):
    import importlib, subprocess
    commands = importlib.import_module('conduit.commands')
    assert hasattr(commands, 'execute_tool'), "execute_tool missing in conduit.commands"
    func = commands.execute_tool
    names = getattr(func, '__code__', None) and getattr(func.__code__, 'co_names', ()) or ()
    called = {}
    ...
    def spy_run(*args, **kwargs):
        called['called_with'] = ('run', args, kwargs)
        return DummyResult()

    def spy_check_call(*args, **kwargs):
        called['called_with'] = ('check_call', args, kwargs)
        return 0

    def fake_popen(*a, **k):
        called['called_with'] = ('Popen', a, k)
        class P:
            def wait(self): return 0
        return P()

    # patch whichever subprocess API the function appears to use
    if 'run' in names:
        monkeypatch.setattr(subprocess, 'run', spy_run, raising=False)
    elif 'check_call' in names:
        monkeypatch.setattr(subprocess, 'check_call', spy_check_call, raising=False)
    elif 'Popen' in names:
        monkeypatch.setattr(subprocess, 'Popen', fake_popen, raising=False)
    else:
        pytest.skip("execute_tool does not reference known subprocess primitives in its bytecode")

    # Attempt to call execute_tool with a simple command. Accept either successful completion
    # or an exception as long as our spy observed a call.
    try:
        # call with a typical list-of-args; some implementations accept *args or named params,
        # so try a positional call and fall back to calling with a single string.
        try:
            func(['echo', 'hi'])
        except TypeError:
            func('echo hi')
    except Exception:
        # ignore execution exceptions; we only require that the subprocess variant was invoked
        pass

    assert 'called_with' in called, "subprocess primitive was not invoked by execute_tool"
