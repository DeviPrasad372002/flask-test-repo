...
def test_save_commits_and_returns_object(monkeypatch):
    ...
    # monkeypatch the db in the module under test
...
def test_update_sets_attributes_and_commits(monkeypatch):
    ...
    m = Model()
    # update should set existing and new attributes
    ext.update(m, {"x": 42, "y": "added"})

    assert m.x == 42
    assert getattr(m, "y") == "added"
    assert fake_session.committed is True

def test_CRUDMixin_create_and_update_calls_save(monkeypatch):
    from conduit.extensions import CRUDMixin
    import conduit.extensions as ext

    saved = {"count": 0, "last": None}
    def fake_save(obj):
        saved["count"] += 1
        saved["last"] = obj
        return obj
    ...
    # create should construct and call save
    inst = MyModel.create(a=5)
    assert isinstance(inst, MyModel)
    assert getattr(inst, "a") == 5
    assert saved["count"] >= 1 and saved["last"] is inst

    # update should modify attributes and call save again
    saved["count"] = 0
    inst.update({"a": 9, "b": "new"})
    assert inst.a == 9
    assert inst.b == "new"
    assert saved["count"] >= 1
    assert saved["last"] is inst

def test_get_by_id_uses_query_attribute():
    ...
    class FakeQuery:
        ...
        def get(self, pk):
            return self._mapping.get(pk)

    class Model:
        query = FakeQuery({1: "one", 2: "two"})

    assert get_by_id(Model, 1) == "one"
    assert get_by_id(Model, 2) == "two"
    assert get_by_id(Model, 99) is None
