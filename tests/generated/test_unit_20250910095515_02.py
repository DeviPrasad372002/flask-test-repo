...
def test_add_and_remove_tag_updates_article_tags():
    # Import inside test as requested
    from conduit.articles.models import Article, Tags, add_tag, remove_tag

    a = Article()
    # ensure a mutable tag collection
    a.tags = []

    # add a tag and assert collection length increases and tag value present
    add_tag(a, 'alpha')
    assert len(a.tags) == 1

    # accept both common attribute names ('tag' or 'name') on Tag objects
    found = any(getattr(t, 'tag', getattr(t, 'name', None)) == 'alpha' for t in a.tags)
    assert found

    # remove the tag and check length is back to 0
    remove_tag(a, 'alpha')
    assert len(a.tags) == 0

def test_favoritesCount_counts_items_on_article():
    from conduit.articles.models import Article, favoritesCount

    a = Article()
    # support both list and set semantics that implementations may use
    u1 = types.SimpleNamespace(id=1, username='u1')
    u2 = types.SimpleNamespace(id=2, username='u2')

    # try list
    a.favorites = [u1, u2]
    assert favoritesCount(a) == 2

    # try set-like (some implementations may expect a set)
    a.favorites = {u1, u2}
    assert favoritesCount(a) == 2

def test_is_favourite_returns_false_for_missing_user_and_true_when_present():
    from conduit.articles.models import Article, add_tag, is_favourite, favoritesCount

    a = Article()
    # ensure favorites collection exists
    a.favorites = []

    user = types.SimpleNamespace(id=10, username='tester')

    # Initially not favourited
    assert not is_favourite(a, user)

    # Simulate favouriting by manipulating favorites collection in a plausible way
    a.favorites.append(user)
    assert is_favourite(a, user)
    # favoritesCount should reflect the change
    assert favoritesCount(a) == 1

def test_make_and_dump_comment_and_articles_are_json_serializable():
    from conduit.articles.serializers import make_comment, dump_comment, dump_articles
    from conduit.articles.models import Article, Comment

    # Create a simple comment input structure
    raw_comment = {'body': 'hello world'}

    # make_comment may return an object or mapping; handle both
    created = make_comment(raw_comment)
    # dump_comment should produce a serializable mapping/dict
    dumped = dump_comment(created)
    # ensure JSON serializable
    json.dumps(dumped)

    # Now ensure dump_articles handles a list of article-like objects
    a1 = Article()
    a1.title = "T1"
    a2 = Article()
    a2.title = "T2"
    dumped_list = dump_articles([a1, a2])
    # should be JSON serializable (mapping or list)
    json.dumps(dumped_list)
