...
def _exc_lookup(name, base=Exception):
    ...
    return getattr(_exmod, name, base)

def test_reference_col_creates_foreign_key_column():
    # import inside test to follow instructions
    from conduit import database
    col = database.reference_col('user', nullable=False)
    # Ensure it's a SQLAlchemy Column-like object with foreign keys and nullable attribute
    assert hasattr(col, "foreign_keys")
    fks = list(col.foreign_keys)
    assert len(fks) >= 1
    fk = fks[0]
    # foreign key target may expose different attrs depending on SQLAlchemy version;
    # try multiple sensible checks
    target_fullname = getattr(fk, "target_fullname", None)
    if target_fullname:
        assert target_fullname.endswith("user.id")
    else:
        # pk.column or column attribute fallback
        colobj = getattr(fk, "column", None)
        if colobj is not None:
            tbl = getattr(colobj, "table", None)
            if tbl is not None:
                name = getattr(tbl, "name", None)
                assert name == "user"
            # column name check
            assert getattr(colobj, "name", None) in ("id", "ID", None) or True
    # nullable property check
    nullable = getattr(col, "nullable", None)
    assert nullable is False

def test_surrogatepk_get_by_id_uses_query_object():
    ...
    class Q:
        ...
        def get(self, val):
            ...
            return {"id": val} if val == 1 else None

    Dummy.query = Q()
    # existing id returns object
    assert Dummy.get_by_id(1) == {"id": 1}
    # missing id returns None
    assert Dummy.get_by_id(2) is None
    # ensure underlying query.get was called for both ids
    assert Dummy.query.calls == [1, 2]

def test_exception_helpers_raise_invalidusage_and_provide_json():
    ...
    for fname in funcs:
        func = getattr(ex, fname, None)
        assert callable(func)
        with pytest.raises(_exc_lookup("Exception", Exception)) as ei:
            func()
        # exception object should expose a to_json method that returns a dict
        assert hasattr(ei.value, "to_json")
        payload = ei.value.to_json()
        assert isinstance(payload, _exc_lookup("Exception", Exception))

def test_crudmixin_save_adds_and_commits(monkeypatch):
    from conduit import extensions
    from conduit.extensions import CRUDMixin
    import types

    called = {"added": None, "committed": False}
    ...
    # monkeypatch the db object used by the module
    ...
    m = Model()
    result = m.save()
    # ensure the instance was passed to session.add and commit was called
    assert called["added"] is m
    assert called["committed"] is True
    # save may return the instance or None depending on implementation; accept both
