...
def _exc_lookup(name, base):
    # Minimal lookup shim for tests that don't rely on custom exceptions.
    return base

def test_crudmixin_update_and_save_commit_behavior(monkeypatch):
    ...
    inst = Dummy(1)
    # update should set attribute and call save (which adds and commits)
    inst.update(x=2)
    assert inst.x == 2
    assert fake_db.session.calls == [('add', inst), ('commit', True)]

    # reset and test save with commit=False -> add called but not commit
    fake_db.session.clear()
    inst.x = 3
    inst.save(commit=False)
    assert fake_db.session.calls == [('add', inst)]

def test_crudmixin_create_uses_save_and_sets_fields():
    import conduit.extensions as ext
    CRUDMixin = ext.CRUDMixin
    ...
    class Dummy(CRUDMixin):
        ...
        def save(self, commit=True):
            # override to avoid DB dependency
            self._saved = True
            return self

    # create is a classmethod provided by CRUDMixin; it should construct and call save
    created = Dummy.create(name='alice', value=7)
    assert isinstance(created, _exc_lookup("Exception", Exception))
    assert created.name == 'alice'
    assert created.value == 7
    assert getattr(created, '_saved', False) is True

def test_extension_level_update_and_save_functions_call_db(monkeypatch):
    import conduit.extensions as ext

    fake_db = _FakeDB()
    monkeypatch.setattr(ext, 'db', fake_db, raising=True)

    obj = types.SimpleNamespace(name='orig')
    # ext.update signature typically update(obj, commit=True, **kwargs)
    returned = ext.update(obj, name='changed')
    # update should mutate object and, by default, save (add + commit) and return object
    assert obj.name == 'changed'
    assert returned is obj
    assert fake_db.session.calls == [('add', obj), ('commit', True)]

    # test save with commit=False: add only
    fake_db.session.clear()
    ext.save(obj, commit=False)
    assert fake_db.session.calls == [('add', obj)]

def test_surrogatepk_provides_id_and_get_by_id_callable():
    ...
    # ensure the mixin supplies an identity descriptor and a lookup method
    assert hasattr(Model, 'id')
    assert callable(getattr(Model, 'get_by_id', None))

def test_serializers_classes_are_instantiable():
    ...
    for name in names:
        ...
        inst = cls()
        # Basic sanity: instantiation succeeded and object is instance of declared class
