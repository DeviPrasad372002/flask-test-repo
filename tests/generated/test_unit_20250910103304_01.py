...
def test_register_extensions_calls_init_app(monkeypatch):
    appmod = importlib.import_module('conduit.app')
    # discover extension-like attributes that expose init_app
    ext_names = [name for name, obj in vars(appmod).items() if hasattr(obj, 'init_app')]
    if not ext_names:
        pytest.skip("no extension-like attributes with init_app found in conduit.app")
    class FakeExt:
    ...
    for name in ext_names:
        fake = FakeExt(name)
        monkeypatch.setattr(appmod, name, fake)
        fakes[name] = fake
    app = Flask('test_register_extensions')
    # call target
    appmod.register_extensions(app)
    # ensure each fake had its init_app called with our app
    for name, fake in fakes.items():
        assert fake.calls == [app], f"extension {name} did not receive init_app call"

def test_register_shellcontext_registers_processor_and_uses_shell_context(monkeypatch):
    appmod = importlib.import_module('conduit.app')
    class FakeApp:
        def __init__(self):
            self._ctx_fn = None
        def shell_context_processor(self, fn):
            # emulate Flask decorator behaviour
            self._ctx_fn = fn
            return fn
    fake_app = FakeApp()
    # ensure shell_context returns a deterministic dict
    monkeypatch.setattr(appmod, 'shell_context', lambda: {'magic': 42})
    # call target
    appmod.register_shellcontext(fake_app)
    assert callable(fake_app._ctx_fn), "shell_context_processor not registered"
    # invoked function should return the dict from our monkeypatched shell_context
    assert fake_app._ctx_fn() == {'magic': 42}

def test_article_favourite_unfavourite_cycle_modifies_favorites_collection():
    ...
    # provide both common storage names used by different implementations
    setattr(art, 'favorited', set())
    setattr(art, 'favorites', set())
    # simple user-like object with an id attribute (covers implementations that use id or object)
    user = type('U', (), {'id': 1001})()
    # ensure methods exist
    assert hasattr(art, 'favourite'), "Article missing favourite method"
    assert hasattr(art, 'unfavourite'), "Article missing unfavourite method"
    assert hasattr(art, 'is_favourite'), "Article missing is_favourite method"
    assert hasattr(art, 'favoritesCount'), "Article missing favoritesCount method"
    # perform favourite
    art.favourite(user)
    # after favouriting, is_favourite should report True
    assert art.is_favourite(user) is True
    # favoritesCount should reflect one favorite
    assert art.favoritesCount() == 1
    # perform unfavourite and verify removal
