...
def test_crudmixin_save_commits(monkeypatch):
    ...
    # Patch conduit.extensions.db to provide our fake session
    dummy_db = types.SimpleNamespace(session=dummy_session)
    monkeypatch.setattr("conduit.extensions.db", dummy_db, raising=False)

    # Create a minimal subclass instance of CRUDMixin
    class Item(CRUDMixin):
        def __init__(self, name):
            self.name = name
    i = Item("x")

    # Call save() and assert behavior
    returned = i.save()
    assert returned is i
    assert dummy_session.added == [i]
    assert dummy_session.committed == 1

    # Calling save(commit=False) should add but not commit
    j = Item("y")
    returned2 = j.save(commit=False)
    assert returned2 is j
    assert dummy_session.added[-1] is j
    # committed counter unchanged
    assert dummy_session.committed == 1

def test_commands_test_invokes_execute_tool(monkeypatch):
    # Import inside test
    import conduit.commands as commands

    calls = []
    def fake_execute_tool(cmd, *a, **kw):
        calls.append(cmd)
        return 0

    monkeypatch.setattr(commands, "execute_tool", fake_execute_tool, raising=True)

    # Call the click command function directly (should call execute_tool)
    # Many CLI functions accept context or options; call without args.
    commands.test()
    assert any("pytest" in c for c in calls), "expected pytest to be invoked"

def test_commands_lint_invokes_execute_tool(monkeypatch):
    import conduit.commands as commands
    ...
    # Call lint command
    commands.lint()
    assert any("flake8" in c or "pylint" in c or "mypy" in c or "ruff" in c for c in calls), "expected a linter tool to be invoked"

def test_commands_clean_removes_matches(monkeypatch, tmp_path):
    import conduit.commands as commands

    # Prepare fake files and directories
    file_a = tmp_path / "module.pyc"
    file_a.write_text("x")
    cache_dir = tmp_path / "__pycache__"
    cache_dir.mkdir()
    nested_file = cache_dir / "x.cpython-xx.pyc"
    nested_file.write_text("y")

    # Monkeypatch glob to return our created items for expected patterns
    def fake_glob(pattern, recursive=False):
        # Return different results depending on pattern to simulate behavior
        if pattern.endswith("**/*.pyc"):
            return [str(file_a), str(nested_file)]
        if pattern.endswith("**/__pycache__"):
            return [str(cache_dir)]
        # fallback empty
        return []
    monkeypatch.setattr(commands, "glob", types.SimpleNamespace(glob=fake_glob).glob, raising=False)
    # Capture removals
    removed = {"files": [], "dirs": []}
    ...
    monkeypatch.setattr(shutil, "rmtree", fake_rmtree, raising=False)

    # Call clean (may accept options; call without)
    commands.clean()

    # Ensure our files and dirs were requested to be removed
    assert any(str(file_a) == p for p in removed["files"]), "expected .pyc file to be removed"
    assert any(str(cache_dir) == d for d in removed["dirs"]), "expected __pycache__ dir to be removed"
