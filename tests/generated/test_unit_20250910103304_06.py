...
def test_invalidusage_to_json_and_raise():
    from conduit import exceptions
    InvalidUsage = exceptions.InvalidUsage
    # construct and inspect json output
    inst = InvalidUsage("bad request", status_code=400)
    # isinstance should match the custom exception
    assert isinstance(inst, _exc_lookup('InvalidUsage', Exception))
    # to_json exists and includes message and status_code
    data = inst.to_json() if hasattr(inst, "to_json") else inst.to_dict()
    assert isinstance(data, _exc_lookup("Exception", Exception))
    assert data.get("message") == "bad request"
    assert data.get("status_code") == 400
    # raising the exception should be catchable as the custom type
    with pytest.raises(_exc_lookup('InvalidUsage', Exception)):
        raise InvalidUsage("boom")


def test_crudmixin_save_calls_db_session_methods(monkeypatch):
    from conduit import extensions
    # create a dummy model that uses CRUDMixin
    class Dummy(extensions.CRUDMixin):
        pass

    dummy = Dummy()
    calls = {"add": False, "commit": False}

    def fake_add(obj):
        # ensure the object passed is the instance
        calls["add"] = obj is dummy

    def fake_commit():
        calls["commit"] = True

    # patch the db.session used by the mixin
    fake_session = types.SimpleNamespace(add=fake_add, commit=fake_commit)
    fake_db = types.SimpleNamespace(session=fake_session)
    monkeypatch.setattr(extensions, "db", fake_db, raising=False)

    # call save and ensure session.add and session.commit were invoked
    dummy.save()
    assert calls["add"] is True
    assert calls["commit"] is True


def test_config_subclasses_inherit_and_flags():
    ...
    # subclasses should inherit from base Config
    assert issubclass(DevConfig, Config)
    assert issubclass(ProdConfig, Config)
    assert issubclass(TestConfig, Config)

    # reasonable expectations about flags
    # TestConfig should enable testing mode
    assert getattr(TestConfig, "TESTING", False) is True
    # DevConfig usually has debug True
    assert getattr(DevConfig, "DEBUG", False) is True
    # ProdConfig typically disables debug
    assert getattr(ProdConfig, "DEBUG", False) is not True


def test_profileschema_serializes_userprofile_flexibly():
    ...
    # construct UserProfile in a tolerant way
    try:
        profile = UserProfile(username="alice", bio="hello", image="pic.png", following=False)
    except TypeError:
        profile = UserProfile()
        setattr(profile, "username", "alice")
        setattr(profile, "bio", "hello")
        setattr(profile, "image", "pic.png")
        setattr(profile, "following", False)

    schema = Schema()
    dumped = schema.dump(profile)
    assert isinstance(dumped, _exc_lookup("Exception", Exception))
    # accept both direct or wrapped representations
    if "username" in dumped:
        assert dumped["username"] == "alice"
        assert dumped.get("bio") == "hello"
        assert dumped.get("image") == "pic.png"
        assert isinstance(dumped.get("following", False), bool)
    else:
        # maybe schema wraps under 'profile'
        assert "profile" in dumped
        inner = dumped["profile"]
        assert inner["username"] == "alice"
        assert inner.get("bio") == "hello"
        assert inner.get("image") == "pic.png"


def test_userschema_exposes_public_fields_and_hides_sensitive():
    ...
    # create User instance tolerant to constructor signature
    try:
        user = User(username="bob", email="bob@example.com", password="s3cr3t")
    except TypeError:
        user = User()
        setattr(user, "username", "bob")
        setattr(user, "email", "bob@example.com")
        # some models store password hash field differently; set a plausible attr
        setattr(user, "password", "s3cr3t")

    schema = Schema()
    dumped = schema.dump(user)
    assert isinstance(dumped, _exc_lookup("Exception", Exception))
    # username and email should be present
    assert dumped.get("username") == "bob"
    assert dumped.get("email") == "bob@example.com"
    # password should not be present in serialized output (sensitive)
    assert "password" not in dumped and "password_hash" not in dumped.values() and "password_hash" not in dumped.keys() or "password" not in dumped.keys()
