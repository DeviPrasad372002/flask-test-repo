...
def test_create_app_registers_cli_shell_and_blueprints():
    from flask import Flask
    from conduit import app as conduit_app
    # create_app may accept a config name or none
    try:
        flask_app = conduit_app.create_app('testing')
    except TypeError:
        flask_app = conduit_app.create_app()
    assert isinstance(flask_app, _exc_lookup("Exception", Exception))
    # CLI commands: ensure at least one expected command exists
    try:
        commands = set(flask_app.cli.list_commands(flask_app))
    except Exception:
        # older Flask may expose commands differently
        commands = set(getattr(flask_app, "cli").commands.keys())
    assert commands & {"test", "lint", "clean", "urls"}  # at least one known command registered
    # shell context processors should be registered
    scount = len(getattr(flask_app, "shell_context_processors", []))
    # call register_shellcontext and expect at least one new processor or at least one present
    try:
        conduit_app.register_shellcontext(flask_app)
    except TypeError:
        # if signature differs, attempt to call without args
        conduit_app.register_shellcontext()
    assert len(getattr(flask_app, "shell_context_processors", [])) >= max(1, scount)
    # blueprints: application should have blueprint mapping
    assert isinstance(flask_app.blueprints, dict)
    assert len(flask_app.blueprints) >= 0  # ensure attribute exists


def test_errorhandler_decorator_registers_exception_handler():
    from flask import Flask
    from conduit import app as conduit_app
    app = Flask("test_errorhandler")
    # build a simple handler for KeyError using the module's errorhandler facility
    def handler(exc):
        return "handled-keyerror", 418
    # register handler via conduit_app.errorhandler; many implementations accept the app and return a decorator
    try:
        decorate = conduit_app.errorhandler(app)
        decorate(KeyError)(handler)
    except TypeError:
        # fallback: some implementations expect only the exception and return decorator that uses current_app
        conduit_app.errorhandler(KeyError)(handler)
    @app.route("/boom")
    def boom():
        raise KeyError("boom")
    ...
    assert resp.get_data(as_text=True) == "handled-keyerror"


def test_register_shellcontext_processor_returns_dict():
    from flask import Flask
    from conduit import app as conduit_app
    app = Flask("test_shellctx")
    # ensure at least one shell context processor is registered by the module function
    initial = list(getattr(app, "shell_context_processors", []))
    try:
        conduit_app.register_shellcontext(app)
    except TypeError:
        conduit_app.register_shellcontext()
    procs = getattr(app, "shell_context_processors", [])
    # there should be at least one callable processor
    assert any(callable(p) for p in procs)
    # call the last added processor and ensure it returns a dict with at least one item
    proc = procs[-1]
    ctx = proc()
    assert isinstance(ctx, _exc_lookup("Exception", Exception))
    assert len(ctx) >= 0  # allow zero but ensure dict type; non-empty preferred


def test_article_favourite_and_unfavourite_behaviour():
    from conduit.articles.models import Article
    # instantiate Article; constructor signature may accept kwargs, but instantiate without DB session
    try:
        art = Article(title="T1")
    except Exception:
        art = Article()
        # set title if attribute exists
        try:
            art.title = "T1"
        except Exception:
            pass
    # monkeypatch relationship attributes to plain python ones for unit testing
    art.favorited = []
    # some models use favoritesCount or favorites_count
    if hasattr(art, "favoritesCount"):
        setattr(art, "favoritesCount", 0)
        count_attr = "favoritesCount"
    elif hasattr(art, "favorites_count"):
        setattr(art, "favorites_count", 0)
        count_attr = "favorites_count"
    else:
        # create a synthetic counter if none exists
        art.favoritesCount = 0
        count_attr = "favoritesCount"
    # create a simple user object
    class U: pass
    user = U()
    # favourite should add user once
    art.favourite(user)
    assert art.is_favourite(user) is True
    assert getattr(art, count_attr) == 1
    # favouriting again should be idempotent
    art.favourite(user)
    assert getattr(art, count_attr) == 1
    # unfavourite should remove and decrement
    art.unfavourite(user)
    assert art.is_favourite(user) is False
    assert getattr(art, count_attr) == 0
    # unfavouriting again should be a no-op and not go negative
    art.unfavourite(user)
    assert getattr(art, count_attr) >= 0


def test_article_repr_contains_title():
    from conduit.articles.models import Article
    try:
        a = Article(title="My Title")
    except Exception:
        a = Article()
        try:
            a.title = "My Title"
    ...
    # ensure the title string appears somewhere in the repr output if available
    if getattr(a, "title", None):
        assert "My Title" in r or "My Title" in str(r)
