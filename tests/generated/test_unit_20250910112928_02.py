...
def _exc_lookup(name, default=Exception):
    ...
    return getattr(excmod, name, default)

def _call_with_optional_user(func, *args):
    sig = inspect.signature(func)
    if len(sig.parameters) == len(args) + 1:
        return func(*args, None)
    return func(*args)

def _extract_article_dict(out):
    if isinstance(out, _exc_lookup("Exception", Exception)) and 'article' in out:
        return out['article']
    if isinstance(out, _exc_lookup("Exception", Exception)) and any(k in out for k in ('title', 'slug', 'body', 'author')):
        return out
    return None

def _extract_comment_dict(out):
    if isinstance(out, _exc_lookup("Exception", Exception)) and 'comment' in out:
        return out['comment']
    if isinstance(out, _exc_lookup("Exception", Exception)) and any(k in out for k in ('id', 'body', 'author', 'createdAt', 'updatedAt')):
        return out
    return None

def test_article_tag_and_favorite_behavior():
    import conduit.articles.models as models
    # instantiate article and ensure tags/favorites structures exist
    art = models.Article()
    # ensure tags attribute exists and is mutable
    if not hasattr(art, 'tags') or art.tags is None:
        if hasattr(models, 'Tags'):
            art.tags = models.Tags()
        else:
            art.tags = []
    # add/remove tag methods
    if hasattr(art, 'add_tag'):
        art.add_tag('python')
        assert 'python' in list(art.tags)
        # adding duplicate should not create multiple entries
        art.add_tag('python')
        assert list(art.tags).count('python') == 1
    else:
        pytest.skip("Article.add_tag not implemented")

    if hasattr(art, 'remove_tag'):
        art.remove_tag('python')
        assert 'python' not in list(art.tags)
    else:
        pytest.skip("Article.remove_tag not implemented")

    # favorites: use a simple user object
    user = SimpleNamespace(id=42, username='bob')
    if hasattr(art, 'favourite') and hasattr(art, 'unfavourite') and hasattr(art, 'is_favourite') and hasattr(art, 'favoritesCount'):
        art.favourite(user)
        assert art.is_favourite(user) is True
        # favorited alias/method if present
        if hasattr(art, 'favorited'):
            assert art.favorited(user) is True
        # count should reflect one favorite
        count = art.favoritesCount()
        assert isinstance(count, _exc_lookup("Exception", Exception)) and count >= 1
        art.unfavourite(user)
        assert art.is_favourite(user) is False
    else:
        pytest.skip("Article favourite/unfavourite/is_favourite/favoritesCount not implemented")

def test_serializers_make_and_dump_article_and_articles_list():
    import conduit.articles.serializers as serializers
    import conduit.articles.models as models
    # create a lightweight author object expected by serializers
    author = SimpleNamespace(username='alice', bio='bio', image='img', following=False)
    # create an Article-like object
    art = models.Article()
    # set common attributes that serializers typically look for
    setattr(art, 'slug', 'alice-article')
    setattr(art, 'title', 'Hello')
    setattr(art, 'description', 'desc')
    setattr(art, 'body', 'content')
    setattr(art, 'tagList', ['x', 'y'])
    setattr(art, 'author', author)
    # dump single article
    if hasattr(serializers, 'dump_article'):
        out = _call_with_optional_user(serializers.dump_article, art)
        assert isinstance(out, _exc_lookup("Exception", Exception))
        article_dict = _extract_article_dict(out)
        assert article_dict is not None
        assert article_dict.get('title') == 'Hello' or article_dict.get('slug') == 'alice-article'
        # author nested
        a_author = article_dict.get('author')
        assert isinstance(a_author, _exc_lookup("Exception", Exception)) and a_author.get('username') == 'alice'
    else:
        pytest.skip("dump_article not implemented")

    # dump list of articles
    if hasattr(serializers, 'dump_articles'):
        out2 = serializers.dump_articles([art, art])
        # expect a dict or list depending on implementation
        assert isinstance(out2, (dict, list))
        if isinstance(out2, _exc_lookup("Exception", Exception)) and 'articles' in out2:
            assert isinstance(out2['articles'], list) and len(out2['articles']) == 2
        elif isinstance(out2, _exc_lookup("Exception", Exception)):
            assert len(out2) == 2
        else:
            pytest.skip("dump_articles returned unexpected structure")
    else:
        pytest.skip("dump_articles not implemented")

    # make_article: should accept a payload and return something serializable or an Article-like dict
    if hasattr(serializers, 'make_article'):
        payload = {'title': 'T', 'description': 'D', 'body': 'B', 'tagList': ['t1']}
        made = serializers.make_article(payload)
        # allow either dict-like result or model instance with attributes
        if isinstance(made, _exc_lookup("Exception", Exception)):
            assert made.get('title') == 'T' or made.get('body') == 'B'
        else:
            assert getattr(made, 'title', None) == 'T' or getattr(made, 'body', None) == 'B'
    else:
        pytest.skip("make_article not implemented")

def test_comment_make_and_dump():
    import conduit.articles.serializers as serializers
    import conduit.articles.models as models
    # construct a minimal comment-like object and author
    author = SimpleNamespace(username='carl', bio=None, image=None, following=False)
    if hasattr(models, 'Comment'):
        com = models.Comment()
    else:
        com = SimpleNamespace()
    setattr(com, 'id', 7)
    setattr(com, 'body', 'nice post')
    setattr(com, 'author', author)
    # dump_comment usage
    if hasattr(serializers, 'dump_comment'):
        out = serializers.dump_comment(com)
        assert isinstance(out, _exc_lookup("Exception", Exception))
        cdict = _extract_comment_dict(out)
        assert cdict is not None
        assert cdict.get('body') == 'nice post' or cdict.get('id') == 7
        a = cdict.get('author')
        assert isinstance(a, _exc_lookup("Exception", Exception)) and a.get('username') == 'carl'
    else:
        pytest.skip("dump_comment not implemented")

    # make_comment should validate or construct a comment-like object/dict
    if hasattr(serializers, 'make_comment'):
        payload = {'body': 'hello'}
        made = serializers.make_comment(payload)
        if isinstance(made, _exc_lookup("Exception", Exception)):
            assert made.get('body') == 'hello'
        else:
            assert getattr(made, 'body', None) == 'hello'
    else:
        pytest.skip("make_comment not implemented")
