...
def test_reference_col_contains_table_and_nullable():
    # import inside test
    db_mod = importlib.import_module('conduit.database')
    reference_col = getattr(db_mod, 'reference_col')
    # call with a table name that is unlikely to be special
    col = reference_col('users', nullable=True)
    # The SQLAlchemy Column / ForeignKey usually renders the referenced target in str()
    s = str(col)
    assert 'users' in s and 'id' in s
    # also check nullable preserved if attribute exists
    if hasattr(col, 'nullable'):
        assert col.nullable is True


def test_get_by_id_returns_value_and_propagates_errors():
    db_mod = importlib.import_module('conduit.database')
    get_by_id = getattr(db_mod, 'get_by_id')
    ...
    # Case 1: query.get returns an object
    sentinel = object()
    class ModelFound:
        query = DummyQuery(result=sentinel)

    res = get_by_id(ModelFound, 1)
    assert res is sentinel

    # Case 2: query.get raises an error -> should propagate
    class ModelErr:
        query = DummyQuery(exc=ValueError("boom"))

    with pytest.raises(_exc_lookup("ValueError", Exception)):
        get_by_id(ModelErr, 1)


def test_execute_tool_invokes_subprocess_check_call(monkeypatch):
    cmds_mod = importlib.import_module('conduit.commands')
    execute_tool = getattr(cmds_mod, 'execute_tool')
    ...
    # patch the subprocess.check_call used at runtime
    monkeypatch.setattr(subprocess, 'check_call', fake_check_call)

    # Try a few common tools until one triggers subprocess invocation
    candidates = []
    if hasattr(cmds_mod, 'TOOLS') and isinstance(cmds_mod.TOOLS, (dict, list, tuple, set)):
        if isinstance(cmds_mod.TOOLS, dict):
            candidates.extend(list(cmds_mod.TOOLS.keys()))
        else:
            candidates.extend(list(cmds_mod.TOOLS))
    candidates.extend(['test', 'lint', 'clean'])

    invoked = False
    last_exc = None
    for cand in candidates:
        try:
            # execute_tool may accept multiple signatures; call with a single name
            execute_tool(cand)
            if called:
                invoked = True
                break
        except Exception as e:
            # record and continue trying other candidates
            last_exc = e
            continue

    assert invoked, "execute_tool did not invoke subprocess for any candidate (last exc: {!r})".format(last_exc)


def test_create_registers_extension_and_calls_init_app():
    ext_mod = importlib.import_module('conduit.extensions')
    create_fn = getattr(ext_mod, 'create')
    ...
    class DummyExt:
        ...
        def init_app(self, app):
            # record that init_app was called with this app
            self.inited_with = app

    app = DummyApp()
    ext = DummyExt()
    # call create with explicit name
    returned = create_fn(app, ext, name='myext')
    assert returned is ext
    assert 'myext' in app.extensions and app.extensions['myext'] is ext
    assert ext.inited_with is app


def test_exception_helpers_either_return_mapping_or_raise_InvalidUsage(_exc_lookup):
    ...
    for name in names:
        if not hasattr(exc_mod, name):
            continue
        fn = getattr(exc_mod, name)
        # try calling with increasing args to accommodate different signatures
        got = None
        try:
            try:
                got = fn()
                ...
                try:
                    ...
                    try:
                        ...
                        # give up calling; consider presence sufficient
                        got = None
        except Exception as e:
            # If it raises an error instance of InvalidUsage, that's also acceptable
            assert isinstance(e, _exc_lookup("Exception", Exception))
            continue
        if got is None:
            # nothing to assert further
            continue
        # acceptable return types: dict-like or an InvalidUsage instance
        if isinstance(got, _exc_lookup("Exception", Exception)):
            assert isinstance(got, _exc_lookup("Exception", Exception))
        else:
            assert isinstance(got, (dict, str)) or hasattr(got, 'to_dict') or hasattr(got, 'message')
