...
def test_config_subclasses_and_testconfig_flag():
    ...
    # ProdConfig should not enable debug
    assert getattr(ProdConfig, "DEBUG", False) is not True

def test_invalidusage_exception_contains_message_and_status():
    exc_cls = _exc_lookup('InvalidUsage', Exception)
    # construct and raise to ensure it's the expected type and carries attributes
    with pytest.raises(_exc_lookup('InvalidUsage', Exception)) as ei:
        raise exc_cls("failure occurred", status_code=499)
    inst = ei.value
    # status_code attribute should be present and reflect value passed
    assert getattr(inst, "status_code", None) == 499
    # message should be discoverable via args or a 'message' attribute or str()
    msg_attr = getattr(inst, "message", None)
    if msg_attr is None:
        # fallback to checking args or str()
        assert "failure occurred" in (inst.args[0] if inst.args else str(inst))
    else:
        assert "failure occurred" in msg_attr

def test_crudmixin_create_update_save_delete_calls_db_session(monkeypatch):
    # import inside test
    ...
    # monkeypatch the module-level db used by CRUDMixin implementation
    ...
    # create should construct and save (calls add & commit)
    inst = Dummy.create(name="first")
    assert isinstance(inst, _exc_lookup("Exception", Exception))
    assert ("add", inst) in fake_db.session.calls
    assert any(c[0] == "commit" for c in fake_db.session.calls)

    # update should change attributes and save again
    fake_db.session.calls.clear()
    inst.update(name="second")
    assert getattr(inst, "name") == "second"
    assert ("add", inst) in fake_db.session.calls or any(c[0] == "commit" for c in fake_db.session.calls)

    # delete should call delete and commit
    fake_db.session.calls.clear()
    inst.delete()
    assert ("delete", inst) in fake_db.session.calls
    assert any(c[0] == "commit" for c in fake_db.session.calls)

def test_profileschema_dump_with_plain_mapping():
    # import inside test
    from conduit.profile.serializers import ProfileSchema
    # Provide a plain mapping representing a profile
    profile_data = {"username": "alice", "bio": "writer", "image": "pic.jpg", "following": False}
    dumped = ProfileSchema().dump(profile_data)
    # marshmallow schemas typically return a dict with keys
    assert isinstance(dumped, _exc_lookup("Exception", Exception))
    assert dumped.get("username") == "alice"
    assert dumped.get("bio") == "writer"
    # image and following may be present depending on schema definition; assert keys exist
    assert "image" in dumped
    assert "following" in dumped

def test_userschema_dump_minimal_keys():
    # import inside test
    from conduit.user.serializers import UserSchema
    user_data = {"username": "bob", "email": "bob@example.com", "bio": "bio", "image": None}
    dumped = UserSchema().dump(user_data)
    assert isinstance(dumped, _exc_lookup("Exception", Exception))
