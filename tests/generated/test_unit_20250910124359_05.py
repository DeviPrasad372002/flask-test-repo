...
def test_tag_and_comment_schemas_serialize():
    """Test with enhanced error handling."""
    try:
        from conduit.articles.serializers import TagSchema, CommentSchema
    except Exception as e:
        pytest.skip(f"Missing serializers: {e}")

    tag_data = {"name": "python"}
    comment_data = {"body": "Nice article", "author": {"username": "alice"}}

    # TagSchema should serialize a simple tag dict
    tag_schema = TagSchema()
    dumped_tag = tag_schema.dump(tag_data)
    assert isinstance(dumped_tag, _exc_lookup("Exception", Exception))
    assert "name" in dumped_tag and dumped_tag["name"] == "python"

    # CommentSchema should serialize a comment dict and keep body
    comment_schema = CommentSchema()
    dumped_comment = comment_schema.dump(comment_data)
    assert isinstance(dumped_comment, _exc_lookup("Exception", Exception))
    assert "body" in dumped_comment and dumped_comment["body"] == "Nice article"

def test_article_schemas_and_meta_exposed():
    """Test with enhanced error handling."""
    try:
        from conduit.articles.serializers import ArticleSchema, ArticleSchemas, Meta
    except Exception as e:
        pytest.skip(f"Missing article serializers: {e}")
    ...
    dumped = a_schema.dump(article_data)
    assert isinstance(dumped, _exc_lookup("Exception", Exception))
    assert dumped.get("title") == "Test"

    # ArticleSchemas should be able to handle a list of articles (many)
    # Implementation may have many=True baked in; allow both usages
    aschemas = ArticleSchemas()
    dumped_list = None
    try:
        dumped_list = aschemas.dump([article_data])
    except TypeError:
        # fallback if ArticleSchemas expects many=True on construction
        aschemas = ArticleSchemas(many=True)
        dumped_list = aschemas.dump([article_data])
    assert isinstance(dumped_list, _exc_lookup("Exception", Exception))
    assert len(dumped_list) == 1
    assert dumped_list[0].get("title") == "Test"

    # Meta should be a serializable/schema-like helper (at least exposed)
    assert callable(Meta) or isinstance(Meta, _exc_lookup("Exception", Exception))

def test_comments_schema_multiple_and_content():
    """Test with enhanced error handling."""
    try:
        from conduit.articles.serializers import CommentsSchema, CommentSchema
    except Exception as e:
        pytest.skip(f"Missing comments serializers: {e}")

    comments = [
        {"body": "first", "author": {"username": "u1"}},
        {"body": "second", "author": {"username": "u2"}},
    ]

    # CommentsSchema should serialize multiple comments
    cs = CommentsSchema()
    try:
        dumped = cs.dump(comments)
    except TypeError:
        # allow CommentsSchema(many=True) pattern
        cs = CommentsSchema(many=True)
        dumped = cs.dump(comments)
    assert isinstance(dumped, _exc_lookup("Exception", Exception))
    assert [c.get("body") for c in dumped] == ["first", "second"]

    # Also ensure individual CommentSchema still works
    single = CommentSchema().dump(comments[0])
    assert single["body"] == "first"

def test_surrogatepk_interface():
    """Test with enhanced error handling."""
    try:
        from conduit.database import SurrogatePK
    except Exception as e:
        pytest.skip(f"Missing SurrogatePK: {e}")

    # SurrogatePK should expose a class-level get_by_id and some id attribute name or descriptor
    assert hasattr(SurrogatePK, "get_by_id") and callable(getattr(SurrogatePK, "get_by_id"))
    # There should be an 'id' attribute defined on the class or in its namespace
    assert ("id" in dir(SurrogatePK)) or hasattr(SurrogatePK, "id")

def test_extensions_update_sets_attributes_and_crudmixin_shape():
    """Test with enhanced error handling."""
    try:
        from conduit import extensions
    except Exception as e:
        pytest.skip(f"Missing extensions module: {e}")

    # update helper should exist and be callable
    assert hasattr(extensions, "update") and callable(getattr(extensions, "update"))

    class Dummy:
        pass

    d = Dummy()
    # Call update to set attributes; many implementations just setattr for each kw
    updated = extensions.update(d, a=1, b="x")
    # Should return the same object (common pattern) and set attributes
    assert updated is d
    assert getattr(d, "a") == 1
    assert getattr(d, "b") == "x"

    # CRUDMixin should be exposed and provide create/update/save methods (or at least attributes)
    if hasattr(extensions, "CRUDMixin"):
        CRUDMixin = extensions.CRUDMixin
        assert hasattr(CRUDMixin, "create")
        assert hasattr(CRUDMixin, "update") or hasattr(CRUDMixin, "save") or callable(getattr(CRUDMixin, "create"))
