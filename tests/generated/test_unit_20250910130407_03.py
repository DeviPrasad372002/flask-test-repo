...
def test_execute_tool_calls_subprocess(monkeypatch):
    ...
    # Patch subprocess.run and subprocess.check_call to record calls
    ...
    # Call execute_tool; ensure at least one subprocess API was invoked
    try:
        execute_tool(["echo", "hello"])
    except Exception:
        # If execute_tool propagates, we still want to ensure our fakes were invoked
        pass
    assert run_called["v"] or check_called["v"], "Neither subprocess.run nor subprocess.check_call was invoked"

def test_commands_delegate_to_execute_tool_and_return_command(monkeypatch):
    """Test with enhanced error handling."""
    try:
    ...
    def fake_execute_tool(cmd):
        captured.append(cmd)
        return cmd
    monkeypatch.setattr(commands_mod, "execute_tool", fake_execute_tool)
    # Many command functions in the module should delegate to execute_tool.
    # Call a few of them and assert they returned the delegated value (the command).
    for fname in ("test", "lint", "clean", "urls"):
        if hasattr(commands_mod, fname):
            fn = getattr(commands_mod, fname)
            try:
                result = fn()
            except TypeError:
                # Some may be click commands expecting ctx; call without args and allow TypeError
                continue
            assert result in captured, f"{fname} did not delegate to execute_tool"

def test_execute_tool_uses_run_and_propagates_return_object(monkeypatch):
    """Test with enhanced error handling."""
    try:
    ...
    monkeypatch.setattr(subprocess, "run", fake_run, raising=False)
    # Also ensure check_call is present but not used in this run
    monkeypatch.setattr(subprocess, "check_call", lambda *a, **k: None, raising=False)
    # execute_tool may return different things; we primarily assert that subprocess.run received our cmd
    try:
        execute_tool(["echo", "it"])
    except Exception:
        # Some implementations re-raise; still assert run was called
        pass
    assert called["args"] is not None, "subprocess.run was not called by execute_tool"

def test_commands_execute_tool_called_by_test_lint_clean_variants(monkeypatch):
    """Test with enhanced error handling."""
    try:
        import conduit.commands as commands_mod
    except Exception:
    ...
    history = []
    def fake_exec(cmd):
        history.append(list(cmd) if isinstance(cmd, (list, tuple)) else [cmd])
        return 0
    monkeypatch.setattr(commands_mod, "execute_tool", fake_exec)
    for name in ("test", "lint", "clean"):
        if hasattr(commands_mod, name):
            fn = getattr(commands_mod, name)
            try:
                result = fn()
            except TypeError:
                # click-decorated commands might require context; skip invocation if so
                continue
            # Ensure execute_tool was invoked and returned our sentinel
            assert history, f"{name} did not call execute_tool"
            last = history[-1]
            assert isinstance(last, _exc_lookup("Exception", Exception)), "execute_tool should be called with a list-like command"
