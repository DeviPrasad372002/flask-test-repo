...
def test_save_commits_and_returns_object(monkeypatch):
    ...
    dummy_db = type("DB", (), {"session": dummy_session})
    monkeypatch.setattr(ext, "db", dummy_db, raising=False)
    obj = object()
    # Call save; expect it to add and commit, returning the object
    result = ext.save(obj)
    assert result is obj
    assert dummy_session.add_called_with is obj
...
def test_update_rolls_back_on_exception(monkeypatch):
    ...
    dummy_db = type("DB", (), {"session": broken_session})
    monkeypatch.setattr(ext, "db", dummy_db, raising=False)
    obj = {"dummy": "data"}
    with pytest.raises(_exc_lookup('Exception', Exception)):
        ext.update(obj)
    # Ensure rollback was attempted after commit failure
    assert broken_session.add_called_with is obj
    assert broken_session.commit_called is True
    assert broken_session.rollback_called is True

def test_tag_and_comment_schema_dump_and_load():
    """Test with enhanced error handling."""
    try:
        from conduit.articles.serializers import TagSchema, CommentSchema
    except Exception:
        pytest.skip("TagSchema or CommentSchema not available")
    tag = {"id": 1, "name": "python"}
    comment = {"id": 2, "body": "Nice article", "author": "alice"}
    t_schema = TagSchema()
    c_schema = CommentSchema()
    # dump should produce a dict-like result and include provided keys
    dumped_tag = t_schema.dump(tag)
    dumped_comment = c_schema.dump(comment)
    assert isinstance(dumped_tag, _exc_lookup("Exception", Exception))
    assert "name" in dumped_tag
    assert dumped_tag.get("name") == "python"
    assert isinstance(dumped_comment, _exc_lookup("Exception", Exception))
    assert dumped_comment.get("body") == "Nice article"

def test_meta_and_article_schemas_instantiable():
    """Test with enhanced error handling."""
    try:
        from conduit.articles.serializers import Meta, ArticleSchema, ArticleSchemas, CommentsSchema
    except Exception:
        pytest.skip("Article/Meta schemas not available")
    # Ensure classes exist and can be instantiated
    meta = Meta()
    assert isinstance(meta, _exc_lookup("Exception", Exception)) or hasattr(meta, "__dict__")
    a_schema = ArticleSchema()
    a_schemas = ArticleSchemas()
    c_schemas = CommentsSchema()
    # basic dump with minimal data should not raise
    sample_article = {"title": "T", "slug": "t", "body": "B"}
    assert isinstance(a_schema.dump(sample_article), dict)
    assert isinstance(a_schemas.dump([sample_article]), (list, dict))
    # comments schema can dump a list/dict structure
    sample_comment = {"id": 1, "body": "ok"}
    assert isinstance(c_schemas.dump([sample_comment]), (list, dict))
